...
server.registerTool("studio_generate_art_style_preview", {
  ...
  inputSchema: {
    projectId: z.string().uuid(),
    styleId: z.string().min(1).max(80).regex(/^[A-Za-z0-9_-]+$/),
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
...
server.registerTool("studio_run_stage", {
  description: "Run one user-triggered high-level Studio stage through Croco Canvas generation-module nodes and the shared runtime. extract_entities always performs a fresh full-script scan with current entities and synchronized-character profiles, then only appends newly returned entities. analyze_storyboard passes existing frames and asks for the smallest viable update while preserving stable IDs and their generated assets. Calls may use configured external providers and incur cost; script edits alone never invoke a model.",
...
server.registerTool("canvas_list_character_generation_tasks", {
  ...
  inputSchema: { characterId: z.string().min(1).max(80), limit: z.number().int().min(1).max(200).default(50) },
  annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
...
server.registerTool("canvas_confirm_character_generation_results", {
  ...
  inputSchema: { generationId: z.string().uuid(), resourceIds: z.array(z.string().min(1).max(80)).min(1).max(20) },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
