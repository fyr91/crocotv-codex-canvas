...
import { generateH3Video, generateImage, generateMusic, generateText, models, type H3GenerationProgress, type TextThinkingMode } from "./providers";
...
function normalizeModel(value: string) { const decoded = value.includes("::") ? value.slice(value.indexOf("::") + 2) : value; const aliases: Record<string, string> = { "volc-doubao-turbo": "doubao-seed-2.1-turbo", "doubao-seed-2-1-turbo-260628": "doubao-seed-2.1-turbo", "volc-deepseek-flash": "deepseek-v4-flash", "deepseek-v4-flash-ga-260731": "deepseek-v4-flash", "deepseek-v4-flash-260425": "deepseek-v4-flash", "volc-deepseek-pro": "deepseek-v4-pro", "deepseek-v4-pro-260425": "deepseek-v4-pro", "bigmodel-glm-52": "glm-5.3", "glm-5.2": "glm-5.3", "bigmodel-glm-5v": "glm-5v-turbo", "runware-gemini-pro": models.runwareLlm[0], "runware-gemini-flash": models.runwareLlm[1], "runware-gemini-flash-lite": models.runwareLlm[2], "runware-lite": models.image[0], "runware-nano": models.image[1], "runware-gpt-image-02": models.image[2], "minimax-h3": "minimax-h3", "minimax-h3-r2v": "minimax-h3", "volc-speech": "volcengine:seed-tts-2.0-expressive", "suno-music": String(models.music) }; return aliases[decoded] || decoded; }
...
function validateModel(mode: GenerationMode, model: string) { if (!model) throw new Error("生成模组必须指定模型"); if (mode === "text" && ![...models.codingPlanLlm, ...models.volcengineLlm, ...models.bigmodelLlm, ...models.runwareLlm].includes(model)) throw new Error(`模型 ${model} 不是可用的文字模型`); if (mode === "image" && !models.image.includes(model)) throw new Error(`模型 ${model} 不是可用的图片模型`); if (mode === "video" && model !== "minimax-h3") throw new Error(`模型 ${model} 不是可用的视频模型`); }
