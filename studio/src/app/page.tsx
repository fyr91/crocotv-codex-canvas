    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 id={titleId} className="font-display text-xl font-medium leading-7 text-foreground">{t("newSeries")}</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="font-medium text-sm text-foreground">{tp("contentScripted")}</span>
                </div>
                <p className="text-sm text-text-secondary leading-relaxed">{tp("contentScriptedDesc")}</p>
                {contentMode === "scripted" && (
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="font-medium text-sm text-foreground">{tp("contentFreeform")}</span>
                </div>
                <p className="text-sm text-text-secondary leading-relaxed">{tp("contentFreeformDesc")}</p>
              </button>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="font-medium text-sm text-foreground">{tp("visualControlR2V")}</span>
                </div>
                <p className="text-sm text-text-secondary leading-relaxed">{tp("visualControlR2VDesc")}</p>
                {defaultGenerationMode === "r2v" && (
                  <span className="recommendation-badge absolute top-2 right-2">
                    {tc("recommended")}
                  </span>
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="font-medium text-sm text-foreground">{tp("visualControlI2V")}</span>
                </div>
                <p className="text-sm text-text-secondary leading-relaxed">{tp("visualControlI2VDesc")}</p>
              </button>
            </div>
          </div>
...
function NewProjectTile({ onClick, episode = false }: { onClick: () => void; episode?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-medium">{episode ? t("newEpisode") : t("newProject")}</span>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-display atelier-display text-base font-medium leading-6 tracking-tight text-foreground truncate">
          {project.title}
        </h3>
        {crumb && (
          <div className="font-mono text-sm uppercase tracking-wider text-text-muted mt-0.5 truncate">
            {crumb}
          </div>
        )}
      </div>
      ...
      {/* Status badge */}
      <span className={`atelier-badge hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-sm font-mono font-medium uppercase tracking-wider flex-shrink-0 ${badge.cls}`}>
        <span className="w-[5px] h-[5px] rounded-full bg-current" />
        {badge.label}
      </span>

      {/* Shot count / duration */}
      <div className="hidden md:flex items-center gap-3 font-mono text-sm text-text-secondary flex-shrink-0">
        ...
        <header className="px-4 md:px-7 pt-5 md:pt-6 pb-3 flex flex-col md:flex-row md:items-end gap-3 md:gap-5">
          ...
          <div className="flex items-center flex-wrap gap-2.5 md:pb-1">
            <button
              ...
              className="glass-button flex items-center gap-2 text-sm font-medium disabled:opacity-50"
            >
            ...
            <div className="relative">
              <button
                ...
                disabled={!online}
                title={!online ? tc("offlineTooltip") : undefined}
                className="bg-primary hover:bg-primary/90 text-on-accent px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition-all text-sm shadow-none disabled:opacity-50"
              >
                <Plus size={14} />
                {t("new")}
                <ChevronDown size={12} />
        ...
        <div className="px-7 pb-2 flex flex-wrap items-center gap-3">
          <div className="inline-flex p-[3px] rounded-full bg-surface-inset atelier-pill-tabs" role="tablist" aria-label={t("statusFilterAria")} onKeyDown={rovingKeyDown}>
            {wsStatusPills.map((pill) => {
              ...
              return (
                ...
                >
                  {pill.label}
                  <span className={`font-mono text-sm ${on ? "text-text-secondary" : "text-text-muted"}`}>{pill.count}</span>
                </button>
              );
            })}
          </div>
          <div className="relative flex-1 min-w-[180px] max-w-[340px] bg-surface-inset border border-glass-border rounded-full atelier-search-input">
            ...
            <input
              ...
              className="w-full bg-transparent border-0 rounded-full py-2 pl-9 pr-4 text-sm text-foreground placeholder-text-muted focus:outline-none"
            />
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-7 pb-10 pt-3">
          {totalCount === 0 ? (
            ...
            >
              <div className="glass-panel atelier-card p-10 rounded-2xl border border-glass-border text-center max-w-[620px] w-full relative overflow-hidden">
                <div className="relative z-[1] flex flex-col items-center gap-4">
                  ...
                  <p className="text-sm text-text-secondary max-w-[440px]">
                    {t("emptyHint")}
                  </p>
                  <div className="flex gap-3 mt-2">
                    <button
                      onClick={() => setIsSeriesDialogOpen(true)}
                      className="bg-primary hover:bg-primary/90 text-on-accent px-5 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-all text-sm shadow-none"
                    >
                      <Plus size={14} />
                      {t("createSeries")}
                    </button>
              ...
              <button
                onClick={syncAll}
                disabled={isSyncing}
                className="mt-5 glass-button flex items-center gap-2 text-sm font-medium disabled:opacity-50"
              >
                <RefreshCw size={14} className={isSyncing ? "animate-spin" : ""} />
                {t("syncFromBackend")}
              </button>
            ...
            >
              ...
              <p className="text-sm font-display atelier-display text-foreground">{t("noMatchTitle")}</p>
              <p className="text-sm text-text-muted mt-1">{tc("noMatchHint")}</p>
              <button
                onClick={() => { setWsStatus("all"); setWsSearch(""); }}
                className="mt-4 glass-button text-sm font-medium"
              >
                {tc("clearFilters")}
              </button>
            </motion.div>
                  ...
                  <section key={`grp-${s.id}`} aria-label={s.title}>
                    <div className="flex items-baseline gap-3 mt-4 mb-4 mx-0.5">
                      <button
                        ...
                        className="font-display atelier-display text-xl font-medium leading-7 tracking-tight text-foreground hover:text-primary transition-colors"
                      >
                        {s.title}
                      </button>
                      <span className="font-mono text-sm uppercase tracking-wider text-text-muted">
                      ...
                      <div className="flex flex-col gap-1.5">
                        ...
                        {!wsFiltering && (
                          ...
                          >
                            ...
                            <span className="text-sm font-medium">{t("newEpisode")}</span>
                          </button>
                        )}
                      </div>
                    ) : (
              ...
              {(() => {
                ...
                <section aria-label={t("standaloneGroup") || "独立项目"}>
                  <div className="flex items-baseline gap-3 mt-6 mb-4 mx-0.5">
                    <span className="font-display atelier-display text-xl font-medium leading-7 tracking-tight text-foreground">
