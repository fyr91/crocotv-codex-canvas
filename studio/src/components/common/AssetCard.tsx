...
export default function AssetCard({ asset, type }: AssetCardProps) {
  ...
  return (
    <div className="glass-panel rounded-xl overflow-hidden relative">
      {isShared ? (
        <span
          className="absolute right-2 top-2 z-10 inline-flex items-center gap-1 rounded-full border border-status-starred-border bg-status-starred-bg px-2 py-[2px] font-mono text-sm font-medium uppercase tracking-[0.18em] text-status-starred-fg shadow-none backdrop-blur-[2px]"
      ...
      <div className="p-3">
        ...
        {asset.description && (
          <p className="text-sm text-text-secondary mt-1 line-clamp-2">{asset.description}</p>
        )}
      </div>
    </div>
  );
