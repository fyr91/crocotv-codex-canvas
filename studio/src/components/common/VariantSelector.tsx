                    ...
                    <>
                        ...
                        <div className="absolute bottom-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="flex items-center gap-1 px-2 py-1 bg-overlay rounded-md backdrop-blur-sm">
                                ...
                                <span className="text-sm text-foreground/80">{t("clickToZoom")}</span>
                            </div>
                        </div>
                    </>
                ) : (
                ...
                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                    {selectedVariant && (
                        <button
                            ...
                            className="p-2 bg-status-failed-bg hover:bg-status-failed-bg text-white rounded-full backdrop-blur-sm"
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 bg-elevated rounded-lg p-1 border border-glass-border">
                        {[1, 2, 3, 4].map(size => (
                            <button
                                ...
                                className={`px-3 py-1 text-sm font-medium rounded-md transition-colors ${batchSize === size
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${isGenerating
                            ...
                            : 'bg-gradient-to-r from-primary to-primary hover:from-primary hover:to-primary text-white shadow-lg shadow-none'
                            }`}
                    >
                        <Layers size={16} />
                        {t("generate")}
                                ...
                                return (
                                    <div
                                        ...
                                        className={`
                                            ...
                                            ${isSelected ? 'border-primary ring-2 ring-primary/30' : isFavorited ? 'border-status-starred-border' : 'border-transparent hover:border-glass-border'}
                                        `}
                                    >
                                        ...
                                        {onFavorite && (
                                            <button
                                                ...
                                                className={`absolute top-1 right-1 p-1 rounded-full transition-all ${isFavorited
                                                    ? 'bg-status-starred-bg text-white'
                                                    : 'bg-overlay/50 text-text-secondary opacity-0 group-hover/variant:opacity-100 hover:bg-status-starred-bg hover:text-white'
                                                    }`}
                                        ...
                                        {!isFavorited && (
                                            <button
                                                ...
                                                className="absolute bottom-1 right-1 p-1 bg-status-failed-bg hover:bg-status-failed-bg rounded-full text-white opacity-0 group-hover/variant:opacity-100 transition-all"
                                                title={t("deleteVariant")}
                                            >
                                                <Trash2 size={10} />
                                            </button>
                                        ...
                                        {isFavorited && (
                                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-background/90 to-transparent py-1 px-1">
                                                <span className="text-sm text-status-starred-fg font-medium">{t("protected")}</span>
                                            </div>
                                        )}
                                    </div>
                                );
