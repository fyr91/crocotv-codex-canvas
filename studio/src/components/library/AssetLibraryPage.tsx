      ...
      <div className="px-4 md:px-7 pb-2 flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium text-text-muted border border-glass-border hover:text-foreground transition-colors"
          >
            <ArrowDownUp size={12} />
                ...
                {sortOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between gap-3 px-3 py-1.5 rounded-lg text-sm font-medium text-left transition-colors ${
                        opt.disabled
                    ...
                    >
                      <span className="flex items-center gap-2">
                        {opt.label}
                        {opt.disabled && (
                          <span className="font-mono text-sm uppercase tracking-[0.06em] text-text-muted px-1.5 py-0.5 rounded-full bg-surface-inset border border-glass-border">
                            {t("sortUsageHint")}
                          </span>
                        )}
                      </span>
        ...
        <div className="flex-1 overflow-y-auto px-7 pb-10 pt-3">
          {loading ? (
            <div className="flex items-center justify-center py-20">
              <div className="text-text-secondary text-sm">{tc("loading")}</div>
            </div>
          ) : counts.all === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              <div className="glass-panel atelier-card p-10 rounded-2xl border border-glass-border text-center max-w-[620px] w-full relative overflow-hidden">
                <div className="relative z-[1] flex flex-col items-center gap-4">
                  ...
                  <p className="text-sm text-text-secondary max-w-[440px]">{t("noAssetsHint")}</p>
                </div>
              </div>
            </div>
          ) : groups.length === 0 ? (
              ...
              {groups.map((grp) => (
                <div key={grp.key}>
                  ...
                  <div className="flex items-baseline gap-3 mb-4">
                    <span className="text-xl font-display atelier-display font-medium text-foreground tracking-tight">{grp.title}</span>
                    <span className="font-mono text-sm text-text-muted tracking-wide uppercase">{grp.meta}</span>
                      ...
                      return (
                        ...
                        >
                          <div className={`${isChar ? "aspect-[4/3]" : "aspect-square"} bg-surface-inset overflow-hidden relative`}>
                            {url ? (
                              ...
                              >
                                ...
                                />
                                <span className="relative font-display atelier-display text-4xl font-medium leading-none select-none text-foreground/90">
                          ...
                          <div className="p-3">
                            ...
                            {viewAxis === "type" ? (
                              <div className="text-sm text-text-muted truncate mt-0.5">{src.name}</div>
                            ) : (
                              asset.description && <div className="text-sm text-text-muted truncate mt-0.5">{asset.description}</div>
                            )}
                          </div>
                        </div>
                      );
