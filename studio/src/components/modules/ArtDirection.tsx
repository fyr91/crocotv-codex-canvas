    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-8 space-y-8 bg-surface">
                ...
                {inSeries && !seriesBaselineLoading && (
                    <>
                        ...
                        {isOverridden && (
                            <div className="flex items-center gap-3 rounded-lg border border-status-processing-border bg-status-processing-bg px-4 py-3">
                                <RotateCcw size={16} className="text-status-processing-fg shrink-0" />
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm text-foreground">
                                        <span className="text-status-processing-fg">{ta("overridingBaseline")}</span>{" "}
                                        <span className="text-text-secondary text-sm">
                                            ({ta("baselineLabel")}: {seriesBaseline?.name})
                                        </span>
                                    </p>
                                    <p className="text-sm text-text-muted mt-0.5">{ta("overrideHint")}</p>
                                </div>
                                <WorkflowActionButton
                                    variant="secondary"
                                    size="sm"
                        ...
                        {canPromote && (
                            <div className="flex items-center gap-3 rounded-lg border border-primary/30 bg-primary/10 px-4 py-3">
                                <ArrowUp size={16} className="text-primary shrink-0" />
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm text-foreground">{ta("promotePromptTitle")}</p>
                                    <p className="text-sm text-text-muted mt-0.5">{ta("promotePromptHint")}</p>
                                </div>
                                <WorkflowActionButton
                                    variant="secondary"
                                    size="sm"
                ...
                {customStyles.length > 0 && (
                    <div>
                        <h3 className="text-xl font-medium text-foreground mb-4 flex items-center gap-2">
                            <Plus size={20} className="text-status-completed-fg" />
                            {ta("customStyles")}
                        </h3>
                        <div className="grid grid-cols-3 gap-3">
            ...
            {pendingOverrideStyle && (
                ...
                >
                    <div
                        className="w-full max-w-md rounded-2xl border border-glass-border bg-elevated shadow-2xl"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <header className="flex items-center justify-between gap-3 px-5 py-3 border-b border-glass-border">
                            <div className="flex items-center gap-2">
                                <AlertTriangle size={15} className="text-status-processing-fg" />
                        ...
                        <div className="px-5 py-4 space-y-3">
                            ...
                            <div className="rounded-lg border border-glass-border bg-glass px-3 py-2 space-y-1">
                                <p className="font-mono text-sm uppercase tracking-[0.18em] text-text-muted">{ta("overrideFromTo")}</p>
                                <p className="text-sm text-foreground">
                                    <span className="text-text-secondary">{seriesBaseline?.name ?? "—"}</span>
                                    <span className="mx-2 text-text-muted">→</span>
                                    <span className="font-medium text-status-processing-fg">{pendingOverrideStyle.name}</span>
                                </p>
                            </div>
                            <p className="text-sm text-text-muted">{ta("overrideConfirmFooter")}</p>
                        </div>
        ...
        >
            <div className="p-4 space-y-2">
                ...
                <div className="flex items-start gap-2">
                    <div className="shrink-0 mt-0.5 flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-hover-bg border border-border-strong">
                        <Sparkles size={9} className="text-foreground" />
                ...
                {/* Reason */}
                {(style as any).reason && (
                    <p className="text-sm text-foreground leading-relaxed line-clamp-2 border-l-2 border-border-strong pl-2">
                        {(style as any).reason}
                    </p>
                )}
            </div>
    ...
    return (
        ...
        >
            <motion.div
                ...
                className="w-[70vw] max-w-[1100px] min-w-[700px] max-h-[90vh] rounded-2xl border border-glass-border bg-elevated shadow-2xl overflow-hidden flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <header className="flex items-center justify-between px-6 py-4 border-b border-glass-border shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-hover-bg border border-border-strong">
                ...
                <div className="flex-1 min-h-0 flex overflow-hidden">
                    ...
                    <div className="w-[38%] shrink-0 bg-glass border-r border-glass-border p-6 flex flex-col justify-center">
                        ...
                        <div>
                            <p className="font-mono text-sm uppercase tracking-[0.18em] text-text-muted mb-3">{ta("keywordsLabel") || "Keywords"}</p>
                            <div className="flex flex-wrap gap-2">
                                {keywords.map((kw, i) => (
                                    <span key={i} className="text-sm px-2.5 py-1 rounded-md bg-hover-bg text-foreground border border-border-strong">
                                        {kw}
                                    </span>
                                ))}
                            </div>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                {!editing && (
                                    <button
                                        ...
                                        className="flex items-center gap-1 text-sm text-text-muted hover:text-foreground transition-colors"
                                    >
                                        <Pencil size={10} />
                                        {ta("customizeBtn") || "自定义"}
                                    </button>
                            ...
                            {editing ? (
                                <textarea
                                    ...
                                    className="w-full h-32 rounded-lg border border-glass-border bg-elevated px-3 py-2.5 text-sm text-foreground leading-relaxed resize-none focus:outline-none focus:border-primary/50 custom-scrollbar"
                                />
                            ) : (
                        ...
                        <div>
                            ...
                            {editing ? (
                                <textarea
                                    ...
                                    className="w-full h-24 rounded-lg border border-glass-border bg-elevated px-3 py-2.5 text-sm text-foreground leading-relaxed resize-none focus:outline-none focus:border-status-failed-border custom-scrollbar"
                                />
                            ) : (
                                <div className="rounded-lg border border-glass-border bg-glass px-3 py-2.5">
                                    <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap">
                ...
                <footer className="flex items-center justify-between px-6 py-3 border-t border-glass-border shrink-0">
                    <div className="text-sm text-text-muted">
                        {isCustomized && (
                            <span className="text-status-processing-fg">{ta("modifiedLabel") || "已修改"}</span>
                        )}
                    </div>
                    <div className="flex items-center gap-2">
            ...
            >
                    ...
                    <div className="p-6 space-y-5 overflow-y-auto">
                        ...
                        {preset.best_for && preset.best_for.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {preset.best_for.map((tag, i) => (
                                    <span key={i} className="text-sm px-2.5 py-1 rounded-md bg-status-completed-bg text-status-completed-fg border border-status-completed-border">
                                        {tag}
                                    </span>
                                ))}
                                {preset.avoid_for?.map((tag, i) => (
                                    <span key={`avoid-${i}`} className="text-sm px-2.5 py-1 rounded-md bg-status-failed-bg text-status-failed-fg border border-status-failed-border line-through">
                                        {tag}
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <p className="text-sm uppercase tracking-wider text-text-muted font-medium">提示词</p>
                                {!editing && (
                                    <button
                                        onClick={onStartEditing}
                                ...
                                {editing && isCustomized && (
                                    <span className="text-sm text-status-processing-fg font-medium">已修改</span>
                                )}
                            </div>
                        ...
                        {preset.sample_prompt && !editing && (
                            <div>
                                <p className="text-sm uppercase tracking-wider text-text-muted mb-1.5">示例描述</p>
                                <p className="text-sm text-text-secondary/70 leading-relaxed italic">
                ...
                {sameCategoryPresets.length > 0 && (
                    <div className="border-t border-glass-border px-6 py-3 shrink-0">
                        <p className="text-sm uppercase tracking-wider text-text-muted mb-2">同类风格</p>
                        <div className="flex gap-2.5 overflow-x-auto pb-1">
                            {sameCategoryPresets.slice(0, 5).map(p => (
                                ...
                                >
                                    ...
                                    <p className="text-sm text-text-muted px-1.5 py-1 truncate">{p.name_zh}</p>
                                </button>
                            ))}
                        </div>
                    </div>
