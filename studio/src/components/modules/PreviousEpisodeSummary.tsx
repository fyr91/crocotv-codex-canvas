                ...
                {loading ? (
                    ...
                    <div className="p-6">
                        <div className="rounded-lg border border-status-failed-border/40 bg-status-failed-bg/50 px-4 py-3 flex items-start gap-2.5">
                            ...
                            <div>
                                <p className="text-sm text-status-failed-fg font-medium">{t("loadFailed")}</p>
                                <p className="text-sm text-status-failed-fg/80 mt-1 break-all">{error}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="px-5 py-5 space-y-5">
                        ...
                        <section className="space-y-2">
                            <div className="flex items-center justify-between gap-2">
                                <h4 className="font-mono text-sm font-medium uppercase tracking-[0.18em] text-text-muted inline-flex items-center gap-1.5">
                                    <Sparkles size={11} />
                                    {t("aiSummary")}
                                </h4>
                                {data.ai_summary && data.ai_summary_stale && (
                                    <span className="font-mono text-sm uppercase tracking-[0.14em] text-status-processing-fg">
                                        {t("stale")}
                                    </span>
                                )}
                            </div>
                            {!data.ai_summary ? (
                                ...
                                <div className="rounded-lg border border-primary/40 bg-primary/[0.08] px-3.5 py-3 space-y-2">
                                    <textarea
                                        ...
                                        rows={6}
                                        className="w-full bg-transparent text-foreground text-sm leading-relaxed resize-none focus:outline-none"
                        ...
                        <section className="space-y-2">
                            <h4 className="font-mono text-sm font-medium uppercase tracking-[0.18em] text-text-muted">
                                {t("rawSnippet")}
                            </h4>
                            <div className="rounded-lg border border-glass-border bg-black/30 px-3.5 py-3">
                                <p className="text-sm leading-relaxed text-text-secondary whitespace-pre-wrap font-mono">
                                    …{data.raw_snippet}
                                </p>
                            </div>
                        </section>
...
function NextHookSection({ hookData, generating, editing, draft, saving, onGenerate, onEditStart, onEditCancel, onDraftChange, onSave }: NextHookSectionProps) {
    ...
    return (
        <section className="space-y-2 pt-4 border-t border-glass-border">
            <div className="flex items-center justify-between gap-2">
                <h4 className="font-mono text-sm font-medium uppercase tracking-[0.18em] text-primary/80 inline-flex items-center gap-1.5">
                    <Sparkles size={11} />
                    {t("title")}
                </h4>
            ...
            {!hookData.hook && !editing ? (
                ...
                <div className="rounded-lg border border-primary/30 bg-primary/[0.06] px-3.5 py-3 space-y-2">
                    <p className="text-sm leading-relaxed text-foreground whitespace-pre-wrap">
                        {hookData.hook}
                    </p>
                    <div className="flex items-center justify-between gap-2">
