        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 pt-4 border-t border-glass-border">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="font-medium text-foreground text-sm">{tp("aspectRatios")}</h3>
                </div>
                    ...
                    <div className="grid grid-cols-5 gap-1">
                        {['9:16', '3:4', '1:1', '4:3', '16:9'].map((ratio) => (
                            <button
                                ...
                                className={`px-2 py-1.5 rounded text-sm border transition-all font-medium ${characterAspectRatio === ratio
                    ...
                    <div className="grid grid-cols-5 gap-1">
                        {['9:16', '3:4', '1:1', '4:3', '16:9'].map((ratio) => (
                            <button
                                ...
                                className={`px-2 py-1.5 rounded text-sm border transition-all font-medium ${sceneAspectRatio === ratio
                                    ? 'bg-primary/20 text-primary border-primary/30'
                                    : 'bg-glass text-text-secondary border-glass-border hover:bg-hover-bg'
                                    }`}
                            >
                    ...
                    <div className="grid grid-cols-5 gap-1">
                        {['9:16', '3:4', '1:1', '4:3', '16:9'].map((ratio) => (
                            <button
                                ...
                                className={`px-2 py-1.5 rounded text-sm border transition-all font-medium ${propAspectRatio === ratio
                                    ? 'bg-primary/20 text-primary border-primary/30'
                                    : 'bg-glass text-text-secondary border-glass-border hover:bg-hover-bg'
                                    }`}
                            >
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-medium text-foreground text-sm">{tp("artDirectionStyle")}</h3>
            </div>

            {artDirectionStyle ? (
                <div className="space-y-3">
                    <div>
                        <label className="text-sm font-medium text-text-muted uppercase mb-1.5 block">{tp("styleName")}</label>
                        <div className="text-sm font-medium text-foreground bg-gradient-to-r from-primary/20 to-primary/20 p-2.5 rounded-lg border border-glass-border">
                ...
                <div className="bg-glass border border-glass-border rounded-lg p-3 text-center">
                    <p className="text-sm text-text-muted mb-2">{tp("noStyleConfigured")}</p>
...
function StoryboardInspector() {
    ...
    if (!selectedFrame) {
        return (
            <div className="space-y-6">
                <div className="p-4 bg-glass rounded-lg border border-glass-border text-center text-text-muted text-sm">
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Action Description */}
            <div className="space-y-2">
                <label className="text-sm font-medium text-text-muted uppercase">{tp("actionDesc")}</label>
                <textarea
            ...
            {/* Dialogue */}
            <div className="space-y-2">
                <label className="text-sm font-medium text-text-muted uppercase">{tp("dialogueContent")}</label>
                <textarea
                                    ...
                                    {currentProject?.characters?.map((char: any) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex items-center gap-2 p-2 rounded border text-sm transition-all ${isSelected
                                ...
                                {selectedChars && selectedChars.length > 0 && (
                                    <div className="space-y-1">
                                        {selectedChars.map((char: any) => (
                                            <div key={char.id} className="bg-glass p-2 rounded text-sm text-text-secondary italic border border-border-subtle">
                                                <span className="font-medium not-italic text-text-muted">{char.name}: </span>
                                                {char.description}
                                            </div>
                                        ))}
                                    </div>
                                            ...
                                            return (
                                                <div className="space-y-1">
                                                    {selectedProps.map((prop: any) => (
                                                        <div key={prop.id} className="bg-glass p-2 rounded text-sm text-text-secondary italic border border-border-subtle">
                                                            <span className="font-medium not-italic text-text-muted">{prop.name}: </span>
                                                            {prop.description}
                                                        </div>
                                                    ))}
                                                </div>
            ...
            {/* Camera Controls */}
            <div className="space-y-2">
                <label className="text-sm font-medium text-text-muted uppercase">{tp("sceneContext")}</label>
                <div className="grid grid-cols-1 gap-2">
                    <select
                        className="bg-input-bg border border-glass-border rounded p-2 text-sm text-text-secondary focus:outline-none"
            ...
            <div className="space-y-2">
                <div className="flex justify-between items-center">
                    <label className="text-sm font-medium text-text-muted uppercase">{tp("imagePrompt")}</label>
                    <button
                ...
                {polishedPrompt && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="bg-primary/20 border border-primary/30 rounded-lg p-3 mt-2 space-y-3"
                    >
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-glass rounded-full overflow-hidden">
                        <div
                            className="h-full bg-status-completed-bg transition-all duration-500"
...
function StatBox({ label, value }: { label: string, value: string | number }) {
    return (
        <div className="bg-glass border border-glass-border rounded p-2 text-center">
            ...
            <div className="text-sm text-text-muted uppercase">{label}</div>
        </div>
    );
}
