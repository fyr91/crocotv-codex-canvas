            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className="relative w-full max-w-2xl max-h-[80vh] flex flex-col rounded-2xl border border-glass-border bg-elevated shadow-2xl"
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-glass-border bg-glass">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <span className="font-sans text-sm font-medium text-foreground truncate">{row.suggestion.local_name}</span>
                    {row.suggestion.suggested_series_id && (
                        <>
                            <span className="font-mono text-sm text-text-muted">→</span>
                            <span className={`font-sans text-sm truncate ${isHighConf ? 'text-foreground' : 'text-text-secondary'}`}>
                                {row.suggestion.suggested_series_name}
                            </span>
                            <span
                                className={`font-mono text-sm px-1.5 py-0.5 rounded-full ${
                                    isHighConf ? "bg-primary/15 text-primary" :
                                    isMediumConf ? "bg-status-processing-bg text-status-processing-fg" :
                    ...
                    {!row.suggestion.suggested_series_id && (
                        <span className="font-mono text-sm px-1.5 py-0.5 rounded-full bg-primary/15 text-primary">
                            {t("new")}
                        </span>
                    )}
                </div>
            ...
            <div className="shrink-0">
                <select
                    ...
                    className="bg-input-bg border border-glass-border rounded px-2 py-1 text-sm text-foreground focus:outline-none focus:border-primary"
                >
                    {row.suggestion.suggested_series_id && (
            ...
            <div className="shrink-0 w-5 grid place-items-center">
                {row.action !== "skip" ? (
                    <Check size={14} className={row.action === "merge_into_series" ? "text-primary" : "text-primary"} />
                ) : (
