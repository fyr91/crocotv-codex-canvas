...
function getFileIcon(ext: string) {
  if (ext === '.fdx') return <FileCode size={24} className="text-primary" />
  if (ext === '.fountain') return <FileText size={24} className="text-status-completed-fg" />
  return <FileText size={24} className="text-text-secondary" />
}
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          {/* Dialog */}
          <motion.div
            className="relative z-10 w-full max-w-lg rounded-2xl border border-foreground/10 bg-elevated/90 p-6 shadow-2xl backdrop-blur-xl"
          ...
          >
            ...
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-medium text-white">{t('dialogs.import.title')}</h2>
              <button
                onClick={handleClose}
            ...
            <div
              ...
              className={`
                ...
                ${isDragOver
                  ...
                  : status === 'error'
                    ? 'border-status-failed-border bg-status-failed-bg'
                    : status === 'success'
                      ? 'border-status-completed-border bg-status-completed-bg'
                      : 'border-foreground/20 bg-foreground/[0.02] hover:border-foreground/30 hover:bg-foreground/[0.04]'
                }
              `}
            >
              <input
              ...
              {status === 'idle' && (
                <>
                  <Upload size={32} className="text-foreground/40 mb-3" />
              ...
              {status === 'uploading' && (
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm text-foreground/50">{t('dialogs.import.parsing')}</p>
                </div>
              )}

              {status === 'success' && (
                <div className="flex flex-col items-center gap-3">
                  <CheckCircle2 size={28} className="text-status-completed-fg" />
                  <div className="flex items-center gap-2">
                    {getFileIcon(ext)}
                    <span className="text-sm text-foreground/80">{selectedFile?.name}</span>
                  </div>
              ...
              {status === 'error' && (
                <div className="flex flex-col items-center gap-3">
                  <AlertCircle size={28} className="text-status-failed-fg" />
                  <p className="text-sm text-status-failed-fg">{errorMsg}</p>
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      reset()
                    }}
                    className="text-sm text-foreground/60 hover:text-white underline mt-1"
                  >
