  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <motion.div
            className="relative w-full max-w-md rounded-xl border border-foreground/10 bg-surface shadow-2xl"
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-foreground/10 px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-medium text-foreground">{t('snapshots.title')}</h2>
              </div>
              <button
                type="button"
                onClick={onClose}
                className="rounded p-1 text-text-secondary transition-colors hover:bg-elevated hover:text-foreground"
              >
                <X size={18} />
              </button>
            </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative flex flex-col items-center">
                          ...
                          {index < snapshots.length - 1 && (
                            <div className="absolute top-3 h-6 w-px bg-foreground/10" />
                          )}
                        </div>
                        <div>
                      ...
                      {confirmingTimestamp === snap.timestamp ? (
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            disabled={restoring}
                            className="rounded-md bg-status-processing-bg px-2.5 py-1 text-sm font-medium text-white transition-colors hover:bg-status-processing-bg disabled:opacity-50"
                          >
                          ...
                          <button
                            ...
                            className="rounded-md border border-foreground/10 px-2 py-1 text-sm text-text-secondary transition-colors hover:text-foreground"
                          >
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 rounded-md border border-foreground/10 px-2.5 py-1 text-sm text-text-secondary opacity-0 transition-all hover:border-foreground/20 hover:text-foreground group-hover:opacity-100"
                        >
                          <RotateCcw size={12} />
            ...
            {confirmingTimestamp && (
              <div className="border-t border-foreground/10 px-5 py-3">
                <div className="flex items-start gap-2 rounded-md bg-status-processing-bg px-3 py-2 text-sm text-status-processing-fg">
