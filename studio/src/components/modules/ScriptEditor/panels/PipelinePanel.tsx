...
function StatCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string | number }) {
  return (
    <div className="flex items-center gap-2.5 rounded-lg bg-elevated/60 border border-foreground/5 px-3 py-2.5">
      <div className="flex h-7 w-7 items-center justify-center rounded bg-hover-bg/50">
        {icon}
      </div>
      <div>
    ...
    <div className="p-3 space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <StatCard
            icon={<Clapperboard size={13} className="text-primary" />}
          ...
          />
          <StatCard
            icon={<Clock size={13} className="text-status-processing-fg" />}
            label={t('panels.statDuration')}
            value={formatDuration(estimatedDuration, t)}
          />
          <StatCard
            icon={<FileText size={13} className="text-status-completed-fg" />}
      ...
      {!isEmbedded && (
        <motion.button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-primary via-primary to-primary px-4 py-3 text-sm font-medium text-white shadow-lg shadow-none hover:shadow-none transition-shadow"
        >
      ...
      <div className="rounded-lg border border-foreground/5 bg-surface/50 p-4">
        <div className="flex items-center gap-2 mb-2">
          <Loader2 size={14} className="text-text-muted" />
          <span className="text-sm font-medium text-text-muted">{t('panels.generationProgress')}</span>
        </div>
        <div className="flex items-center justify-center py-4">
          <p className="text-sm text-text-muted/60 italic">{t('panels.noActiveTasks')}</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-foreground/5 bg-surface/50 p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-medium text-text-muted">{t('panels.recentGeneration')}</span>
        </div>
        <div className="flex items-center justify-center py-4">
          <p className="text-sm text-text-muted/60 italic">{t('panels.noHistory')}</p>
        </div>
      </div>
    </div>
  );
