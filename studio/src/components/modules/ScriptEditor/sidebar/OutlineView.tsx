    ...
    return (
      <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-elevated mb-2">
          <TreePine size={16} className="text-text-muted" />
        </div>
        <p className="text-sm text-text-muted">{t('sidebar.noOutline')}</p>
      </div>
    );
  }
    ...
    <div className="p-2">
      ...
      {outline.flatScenes.map((scene) => (
        <button
          ...
          className="flex w-full items-center gap-2 px-2 py-1.5 text-left rounded hover:bg-foreground/5 transition-colors"
        >
          <Film size={12} className="shrink-0 text-text-secondary" />
          <span className="text-sm text-foreground truncate">
            {scene.number != null && (
              <span className="text-sm font-mono text-text-muted mr-1">#{scene.number}</span>
            )}
            {scene.title}
          </span>
        </button>
      ...
      {outline.sections.map((section) => (
        <div key={section.id} className="mb-1">
          <button
            ...
            className="flex w-full items-center gap-1.5 px-2 py-1.5 text-left rounded hover:bg-foreground/5 transition-colors"
          >
            <motion.div
            ...
            <TreePine size={12} className="shrink-0 text-primary" />
          ...
          <AnimatePresence>
            {!collapsed[section.id] && (
              ...
              >
                <div className="ml-4 border-l border-foreground/5 pl-2">
                  {section.scenes.map((scene) => (
                    <button
                      key={scene.id}
                      type="button"
                      onClick={() => handleJump(scene.pos)}
                      className="flex w-full items-center gap-2 px-2 py-1 text-left rounded hover:bg-foreground/5 transition-colors"
                    >
                      <Film size={10} className="shrink-0 text-text-muted" />
