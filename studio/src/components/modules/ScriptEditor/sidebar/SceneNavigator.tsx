...
function SceneItem({
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        className="shrink-0 w-3 h-3 rounded-full border border-foreground/20 transition-colors hover:border-foreground/40"
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {scene.intExt && (
            <span className="text-sm font-medium uppercase px-1 py-0.5 rounded bg-hover-bg text-text-secondary shrink-0">
              {scene.intExt}
            </span>
          )}
      ...
      {showColors && (
        <div
          className="absolute left-8 mt-1 z-50 p-1.5 bg-elevated border border-foreground/10 rounded-lg shadow-xl grid grid-cols-4 gap-1"
        ...
        >
          {COLOR_OPTIONS.map((c) => (
            <button
              key={c}
              type="button"
              className="w-5 h-5 rounded-full border border-foreground/20 hover:scale-110 transition-transform"
          ...
          <button
            type="button"
            className="w-5 h-5 rounded-full border border-foreground/20 hover:scale-110 transition-transform bg-muted flex items-center justify-center text-sm text-text-secondary"
            onClick={() => {
              onColorChange(null);
              setShowColors(false);
            }}
  ...
  if (derivedScenes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-elevated mb-2">
          <Film size={16} className="text-text-muted" />
        </div>
        <p className="text-sm text-text-muted">{t('sidebar.noScenes')}</p>
      </div>
    );
  }
