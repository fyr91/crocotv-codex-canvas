...
const STATUS_COLORS: Record<string, string> = {
  draft: 'bg-hover-bg text-text-secondary',
  confirmed: 'bg-primary/60 text-primary',
...
export default function StoryboardView({ editor, onShotClick }: StoryboardViewProps) {
  ...
  if (shots.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center text-text-muted">
        ...
        <p className="mt-1 text-sm text-text-muted">{t('views.storyboardEmptyHint')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
          ...
          <AnimatePresence mode="popLayout">
            {shots.map((shot, index) => (
              ...
              >
                ...
                <div className="relative aspect-video w-full rounded-t-lg bg-elevated/80">
                  {shot.thumbnailUrl ? (
                    <img
                  ...
                  {/* Shot number badge */}
                  <div className="absolute left-2 top-2 rounded bg-black/70 px-1.5 py-0.5 text-sm font-medium text-foreground backdrop-blur-sm">
                    #{shot.shotNumber}
                  </div>
                  {/* Drag handle */}
                ...
                <div className="px-3 py-2.5">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <span
                      className={`rounded px-1.5 py-0.5 text-sm font-medium ${STATUS_COLORS[shot.status]}`}
                    >
                      {t(`views.status${shot.status.charAt(0).toUpperCase() + shot.status.slice(1)}` as any)}
                    </span>
                    <span className="flex items-center gap-0.5 text-sm text-text-muted">
                      <Clock size={10} />
      ...
      {/* Bottom stats bar */}
      <div className="flex shrink-0 items-center gap-4 border-t border-foreground/10 bg-surface/80 px-6 py-2.5 text-sm text-text-secondary">
