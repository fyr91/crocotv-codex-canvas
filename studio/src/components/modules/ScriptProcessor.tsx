    ...
    return (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-overlay backdrop-blur-sm" onClick={onClose}>
            <div className="w-[400px] bg-elevated border border-glass-border rounded-xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
                <h3 className="font-medium text-foreground">{ts("addEntity")}</h3>
                ...
                <div className="flex gap-2 p-1 bg-surface rounded-lg">
                    {(["character", "scene", "prop"] as const).map(t => (
                        <button
                            key={t}
                            onClick={() => setType(t)}
                            className={`flex-1 py-1.5 text-sm font-medium rounded capitalize ${type === t ? "bg-primary text-foreground" : "text-text-muted hover:text-foreground"}`}
                        >
                            {t}
                        </button>
                    ))}
                ...
                <div>
                    <label className="text-sm text-text-muted">{ts("nameLabel")}</label>
                    <input
                ...
                <div>
                    <label className="text-sm text-text-muted">{ts("descriptionLabel")}</label>
                    <textarea
                ...
                <div className="flex justify-end gap-2 pt-2">
                    <button onClick={onClose} className="px-4 py-2 text-sm text-text-secondary hover:text-foreground">{tc("cancel")}</button>
                    <button onClick={handleSubmit} className="px-4 py-2 bg-primary text-foreground rounded text-sm font-medium">{tc("create")}</button>
                </div>
            </div>
        </div>
    );
