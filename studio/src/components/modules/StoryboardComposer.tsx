...
export default function StoryboardComposer() {
    ...
    const handleExtractLastFrame = async (frameId: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            alert(error?.response?.data?.detail || t("extractLastFrameFailed"));
        } finally {
    ...
    const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            alert(error?.message || t("uploadFrameFailed"));
        } finally {
                ...
                trailing={(
                    <div className="flex items-center gap-2">
                        <span className="font-mono text-sm uppercase tracking-[0.18em] text-text-muted">
                            ...
                            <span className="ml-1.5">{t("framesLabel")}</span>
                        </span>
                ...
                <div className="max-w-4xl mx-auto space-y-6">
                        ...
                        {currentProject?.frames?.map((frame: any, index: number) => (
                            <>
                                ...
                                >
                                    {/* Frame Number */}
                                    <div className="absolute -left-3 -top-3 w-8 h-8 rounded-full bg-elevated border border-glass-border flex items-center justify-center text-sm font-medium text-text-secondary shadow-lg z-10">
                                        {index + 1}
                                    </div>
                                    ...
                                    <div className="w-64 aspect-video bg-surface rounded-lg border border-border-subtle overflow-hidden flex-shrink-0 relative">
                                        ...
                                        <div className="absolute inset-0 bg-overlay opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 pointer-events-none">
                                            ...
                                            <button
                                                ...
                                                className="p-2 bg-glass hover:bg-hover-bg text-foreground rounded-lg text-sm font-medium flex items-center gap-1 pointer-events-auto"
                                                ...
                                                <div className="flex items-center gap-1 pointer-events-auto">
                                                    {renderingFrames.has(frame.id) ? (
                                                        <div className="flex items-center gap-2 px-3 py-1.5 bg-elevated rounded-lg">
                                                            ...
                                                            <span className="text-sm text-foreground">{t("generatingFrames")}</span>
                                                        </div>
                                                    ) : (
                                                        <>
                                                            {[1, 2, 3].map(size => (
                                                                <button
                                                                    ...
                                                                    className="px-2 py-1.5 bg-primary/80 hover:bg-primary text-foreground rounded text-sm font-medium transition-colors"
                                                                    title={t("generateVariantsTitle", { count: size })}
                                                                >
                                    ...
                                    <div className="flex-1 flex flex-col gap-3">
                                        <div className="flex items-start justify-between">
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2">
                                                    <span className="font-mono text-sm font-medium text-text-secondary uppercase tracking-[0.18em]">{t("actionLabel")}</span>
                                                    {frame.camera_movement && (
                                                        <span className="font-mono text-sm uppercase tracking-[0.12em] px-1.5 py-0.5 bg-primary/15 text-primary rounded border border-primary/40">
                                                            {frame.camera_movement}
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        {frame.dialogue && (
                                            <div className="mt-auto pt-3 border-t border-border-subtle">
                                                <span className="font-mono text-sm font-medium text-text-secondary uppercase tracking-[0.18em] block mb-1">{t("dialogueLabel")}</span>
                                        ...
                                        <div className="flex justify-end gap-2 mt-2 pt-2 border-t border-border-subtle">
                                            ...
                                            <button
                                                ...
                                                className="btn-tip p-2 hover:bg-status-failed-bg text-text-secondary hover:text-status-failed-fg rounded-lg transition-colors"
                                                data-tip="Delete"
                                            >
                                                <Trash2 size={14} />
                                            </button>
                        ...
                        >
                            <div className="flex items-center justify-between px-6 py-4 border-b border-glass-border bg-surface">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-sm font-medium text-foreground">{t("originalScript")}</h3>
                                </div>
                                <button
                                    onClick={() => setShowScriptOverlay(false)}
            ...
            >
                <div className="p-6 border-b border-glass-border flex justify-between items-center bg-surface">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-medium text-foreground">{t("addNewFrame")}</h2>
                    </div>
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-text-secondary mb-2">{t("actionDescriptionRequired")}</label>
                        <textarea
                            value={action}
...
function ImageWithRetry({ src, alt, className, onClick }: { src: string, alt: string, className?: string, onClick?: (e: React.MouseEvent) => void }) {
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            />
            {error && retryCount >= 10 && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-status-failed-bg backdrop-blur-sm z-20 p-2 text-center">
                    <span className="text-sm text-status-failed-fg font-medium">{t("loadFailed")}</span>
                    <span className="text-sm text-status-failed-fg break-all">{src}</span>
                </div>
            )}
        </div>
    );
