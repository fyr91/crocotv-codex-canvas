...
export default function VideoAssembly() {
    ...
    const handleDownload = async () => {
        ...
        try {
            // Croco resource URLs are served through the Studio same-origin rewrite.
                ...
                <div className="flex items-center gap-1 px-6 pt-2 border-b border-glass-border bg-surface">
                    {[
                        ...
                        <button
                            ...
                            className={`relative inline-flex items-center gap-1.5 px-3 pb-2 font-mono text-sm uppercase tracking-[0.16em] transition-colors ${
                                phase === p.id
                                    ? "text-foreground"
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`group relative flex rounded-xl overflow-hidden cursor-pointer border transition-all bg-glass hover:bg-hover-bg ${isSelected ? "border-primary ring-1 ring-primary/50" :
                                        selectedVideoId ? "border-border-strong" : "border-glass-border"
                                        }`}
                                >
                                    {/* Left: Preview */}
                                    <div className="w-48 aspect-video relative flex-shrink-0 border-r border-glass-border bg-elevated">
                                        {selectedVideo ? (
                                            ...
                                            <div className="w-full h-full relative">
                                                ...
                                                <div className="absolute inset-0 flex items-center justify-center">
                                                    {hasVideos ? (
                                                        <div className="bg-status-starred-bg text-status-starred-fg px-2 py-1 rounded text-sm font-medium border border-status-starred-border">
                                                            {ta("selectVideo")}
                                                        </div>
                                                    ) : (
                                                        <div className="bg-status-failed-bg text-status-failed-fg px-2 py-1 rounded text-sm font-medium border border-status-failed-border">
                                                            {ta("noVideos")}
                                                        </div>
                                                    )}
                                                </div>
                                        ...
                                        <div className="absolute top-2 left-2 bg-surface px-2 py-0.5 rounded text-sm font-mono text-foreground">
                                            #{index + 1}
                                        </div>
                                    </div>
                                ...
                                {variants.length > 0 ? (
                                    variants.map((video: any, idx: number) => {
                                        ...
                                        return (
                                            <div
                                                key={video.id}
                                                className={`rounded-xl overflow-hidden border transition-all group ${isSelected ? "border-border-strong ring-1 ring-border-strong bg-hover-bg" : "border-glass-border bg-glass hover:border-glass-border"
                                                    }`}
                                            >
                                                <div className="aspect-video relative bg-black">
                                                    ...
                                                    />
                                                    {/* Overlay Info */}
                                                    <div className="absolute top-2 left-2 bg-surface px-1.5 rounded text-sm text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                                                        {video.duration}s
                                                    </div>
                                                </div>
                                                <div className="p-3">
                                                    ...
                                                    {isSelected ? (
                                                        <div className="w-full py-2 bg-status-completed-bg text-status-completed-fg rounded-lg text-sm font-medium flex items-center justify-center gap-2 border border-status-completed-border">
                                                            <Check size={14} /> {ta("selected")}
                                                        </div>
                                                    ) : (
                                                        <button
                                                            onClick={() => handleSelectVideo(selectedFrameId, video.id)}
                                                            className="w-full py-2 bg-hover-bg hover:bg-hover-bg rounded-lg text-sm font-medium transition-colors text-foreground"
                                                        >
                                    ...
                                    <div className="text-center py-12 text-text-muted flex flex-col items-center">
                                        ...
                                        <p className="text-sm mt-1 max-w-[200px]">{ta("noVideosHint")}</p>
                                    </div>
                                )}
                            </div>
                        ) : (
...
function MixPhase({
    ...
    return (
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8">
            {/* BGM picker */}
            <section>
                <h3 className="mb-3 flex items-center gap-2 font-mono text-sm uppercase tracking-[0.18em] text-text-muted">
                    <Music size={12} className="text-primary" />
                    {ta("mixBgmTitle")}
                    {saving && <Loader2 size={12} className="animate-spin text-primary" />}
                </h3>
                {/* Croco exposes local audio/music resources as BGM presets and
                    mixes the selected resource during the canonical merge. */}
                <div className="mb-3 flex items-start gap-2 rounded-md border border-status-processing-border bg-status-processing-bg px-3 py-2">
                    <AlertTriangle size={13} className="mt-0.5 shrink-0 text-status-processing-fg" aria-hidden="true" />
                    <p className="text-sm leading-relaxed text-status-processing-fg">
                        ...
                        presets.map((p) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p className="mt-0.5 font-mono text-sm uppercase tracking-[0.14em] text-text-muted">{p.mood}</p>
                                </button>
                            );
                        })
                    )}
                ...
                <div className="space-y-3 max-w-lg">
                    {(["dialogue", "bgm", "sfx"] as const).map((track) => (
                        <div key={track} className="flex items-center gap-3">
                            <span className="w-20 font-mono text-sm uppercase tracking-[0.16em] text-text-muted">{ta(`mixTrack.${track}`)}</span>
                            <input
                                type="range"
                                min={0}
                                max={100}
                            ...
                            />
                            <span className="w-12 text-right font-mono text-sm text-text-secondary">{mix[track] ?? 0}</span>
                        </div>
                    ))}
                </div>
...
function ExportPhase({
    ...
    return (
        <div className="space-y-6 max-w-3xl">
            <section className="rounded-xl border border-glass-border bg-glass p-6">
                <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                        ...
                        className="shrink-0 inline-flex items-center gap-2 bg-primary text-white border border-primary/65 shadow-none hover:bg-primary-hover disabled:opacity-40 disabled:cursor-not-allowed px-5 py-2.5 rounded-md font-medium text-sm"
                    >
            ...
            {mergeError && (
                <div className="rounded-xl border border-status-failed-border bg-status-failed-bg p-4">
                    <div className="flex items-start gap-3">
                        <AlertTriangle className="text-status-failed-fg flex-shrink-0 mt-0.5" size={20} />
                        <div className="flex-1 min-w-0">
                            <h4 className="text-sm font-medium text-status-failed-fg mb-1">{ta("mergeFailed")}</h4>
                            <p className="text-sm text-status-failed-fg whitespace-pre-wrap leading-relaxed font-mono break-all">
                                {mergeError}
                            </p>
                    ...
                    >
                        <div className="grid md:grid-cols-2 gap-0">
                            ...
                            <div className="p-5 flex flex-col justify-center gap-3">
                                <div>
                                    <h3 className="text-display font-medium text-foreground flex items-center gap-2">
                                        <Check className="text-status-completed-fg" size={16} />
                                        {ta("mergedVideoReady")}
                                    </h3>
                                ...
                                <button
                                    ...
                                    className="self-start inline-flex items-center gap-2 px-5 py-2.5 rounded-md bg-glass border border-glass-border text-foreground hover:bg-hover-bg transition-colors text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    <Download size={14} />
