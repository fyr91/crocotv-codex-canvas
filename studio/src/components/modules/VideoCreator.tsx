...
export default function VideoCreator({ onTaskCreated, remixData, onRemixClear, params, onParamsChange }: VideoCreatorProps) {
    ...
    const handleExtractLastFrame = async (frameId: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            alert(error?.response?.data?.detail || tc("extractLastFrameFailed"));
        } finally {
        ...
        <div className="h-full flex flex-col relative min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar min-h-0">
                <h2 className="text-xl font-display font-medium text-foreground mb-6 flex items-center gap-3">
                    ...
                    {tc("title")}
                    <span className="text-sm font-mono text-text-muted bg-glass px-2 py-1 rounded">{tc("motionBadge")}</span>
                </h2>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex bg-glass rounded-lg p-1 gap-1">
                                    <button
                                        ...
                                        className={`px-3 py-1.5 text-sm rounded-md flex items-center gap-2 transition-all ${activeTab === "storyboard"
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1.5 text-sm rounded-md flex items-center gap-2 transition-all ${activeTab === "upload"
                                            ? "bg-primary text-foreground shadow-sm"
                                            : "text-text-secondary hover:text-foreground hover:bg-glass"
                                            }`}
                                    >
                                    ...
                                    <div className="space-y-4">
                                        {currentProject?.frames && currentProject.frames.length > 0 ? (() => {
                                                ...
                                                {currentProject.frames.map((frame: any, index: number) => {
                                                    ...
                                                    >
                                                        {(frame.rendered_image_url || frame.image_url) ? (
                                                            ...
                                                            <div className="w-full h-full bg-glass flex items-center justify-center text-sm text-text-muted">
                                                                {tc("noImage")}
                                                            </div>
                                                        )}
                                                        <div className="absolute inset-0 bg-overlay opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                            <span className="text-sm text-foreground font-medium">{tc("select")}</span>
                                                        </div>
                                                        {/* Frame Number Badge */}
                                                        <div className="absolute top-1 left-1 bg-surface px-1.5 rounded text-sm text-text-secondary backdrop-blur-sm">
                                                            #{frame.id.slice(0, 4)}
                                                        </div>
                                                        ...
                                                        {prevVideoCompleted && (
                                                            <button
                                                                ...
                                                                className={`absolute bottom-1 right-1 flex items-center gap-1 px-1.5 py-0.5 rounded text-sm font-medium backdrop-blur-sm transition-colors ${
                                                                    hasExtracted
                                                                        ? "bg-status-completed-bg text-status-completed-fg border border-status-completed-border hover:bg-primary/20 hover:text-primary hover:border-primary/30"
                                                            ...
                                                            >
                                                                {isExtracting ? (
                                                                    ...
                                                                    <><Film size={10} /> {tc("previousEndFrame")}</>
                                                                )}
                                                            </button>
                                                        )}
                                                    </div>
                                            ...
                                            <div className="flex flex-col items-center justify-center h-[200px] text-text-muted gap-2">
                                                ...
                                                <p className="text-sm">{tc("noStoryboardFrames")}</p>
                                            </div>
                                        )}
                                        ...
                                        {selectedImages.length > 0 && (
                                            <div className="pt-4 border-t border-glass-border">
                                                <p className="text-sm text-text-muted mb-2">{tc("selectedForGeneration")}</p>
                                                        ...
                                                        return (
                                                            <div key={idx} className="relative w-24 aspect-video rounded-lg overflow-hidden border border-glass-border">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="absolute top-1 right-1 p-0.5 bg-surface rounded-full text-foreground hover:bg-status-failed-bg"
                                                                >
                                                                    <X size={10} />
                                                                </button>
                                                            </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-3 gap-4">
                                            {selectedImages.map((img, idx) => (
                                                <div key={idx} className="relative aspect-video bg-surface rounded-xl overflow-hidden border border-glass-border group">
                                                    ...
                                                    <button
                                                        onClick={() => removeImage(idx)}
                                                        className="absolute top-2 right-2 p-1 bg-surface rounded-full text-foreground opacity-0 group-hover:opacity-100 transition-opacity hover:bg-status-failed-bg"
                                                    >
                                                        <X size={12} />
                                                    </button>
                                            ...
                                            >
                                                ...
                                                <p className="text-text-secondary text-sm font-medium">{tc("addImage")}</p>
                                            </div>
                                        </div>
                                        ...
                                        {availableAssets.length > 0 && (
                                            <div className="mt-4 pt-4 border-t border-glass-border">
                                                <p className="text-sm text-text-muted mb-2">{tc("quickSelectAssets")}</p>
                    ...
                    {generationMode === 'r2v' && (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                <label className="text-sm font-medium text-text-secondary">{tc("noFrameSelected")}</label>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
                                    {currentProject?.frames && currentProject.frames.length > 0 ? (
                                            ...
                                            >
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-sm text-text-secondary mb-1">#{frame.id.slice(0, 6)}</p>
                                                        <p className="text-sm text-text-secondary line-clamp-2">
                                                            {frame.action_description || frame.image_prompt || 'No description'}
                                                        </p>
                                                        {frame.dialogue && (
                                                            <p className="text-sm text-primary mt-1 italic line-clamp-1">
                                                                “{frame.dialogue}”
                                                            </p>
                                                        )}
                                                    </div>
                                        ...
                                        <div className="col-span-2 flex flex-col items-center justify-center h-[100px] text-text-muted gap-2">
                                            ...
                                            <p className="text-sm">{tc("noFrameSelected")}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                {r2vUsesImages ? (
                                    ...
                                    <>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="absolute top-2 left-2 z-10">
                                                            <span className="text-sm px-2 py-0.5 rounded-full bg-primary text-white font-medium">
                                                        ...
                                                        {slot?.url ? (
                                                            ...
                                                            <div className="aspect-square relative">
                                                                ...
                                                                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 rounded-b-xl">
                                                                    <p className="text-sm text-foreground font-medium truncate">{slot.name}</p>
                                                                </div>
                                                                <button
                                                                    onClick={() => handleClearCastSlot(slotIndex)}
                                                                    className="absolute top-2 right-2 p-1 bg-surface rounded-full text-foreground hover:bg-status-failed-bg transition-colors"
                                                                >
                                                                    <X size={12} />
                                                                </button>
                                                            </div>
                                                            ...
                                                            <div className="aspect-square flex flex-col items-center justify-center p-3">
                                                                ...
                                                                <select
                                                                    className="w-full text-sm bg-input-bg border border-glass-border rounded-lg px-2 py-1.5 text-text-secondary focus:border-primary focus:outline-none"
                                                                    value=""
                                                                    onChange={(e) => {
                                                                ...
                                                                {slotIndex === 0 && (
                                                                    <p className="text-sm text-status-processing-fg mt-1">{tc("required")}</p>
                                                                )}
                                                            </div>
                                                        )}
                                                    </div>
                                        ...
                                        <p className="text-sm text-text-muted">{tc('refImagesHint')}</p>
                                        {availableReferenceImages.length === 0 && (
                                            <p className="text-sm text-status-processing-fg bg-status-processing-bg border border-status-processing-border rounded-lg p-3">
                                    ...
                                    <>
                                        <div className="grid grid-cols-3 gap-4">
                                            {[0, 1, 2].map((slotIndex) => {
                                                const slot = castSlots[slotIndex];
                                                const slotTitle = slotIndex === 0 ? tc("protagonist") : tc("supporting");
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="absolute top-2 left-2 z-10">
                                                            <span className="text-sm px-2 py-0.5 rounded-full bg-primary text-white font-medium">
                                                                {tc("characterSlot", { index: slotIndex + 1 })}
                                                            </span>
                                                        </div>
                                                        ...
                                                        {slot?.url ? (
                                                            ...
                                                            <div className="aspect-video flex flex-col items-center justify-center p-4">
                                                                <p className="text-sm text-text-secondary mb-2">{slotTitle}</p>
                                                                <select
                                                                    className="w-full text-sm bg-input-bg border border-glass-border rounded-lg px-2 py-1.5 text-text-secondary focus:border-primary focus:outline-none"
                                                                    value=""
                                                                    onChange={(e) => {
                                                                ...
                                                                {slotIndex === 0 && (
                                                                    <p className="text-sm text-status-processing-fg mt-2">{tc("required")}</p>
                                                                )}
                                                            </div>
                                                        )}
                                                    </div>
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                {generationMode === 'i2v' && (
                                    <div className="relative">
                                        <button
                                            ...
                                            className="text-sm flex items-center gap-1 px-2 py-1 rounded transition-colors text-text-secondary hover:text-foreground hover:bg-glass"
                                        >
                                            <Video size={12} /> {tc("camera")}
                                        </button>
                                    </div>
                                )}
                                <button
                                ...
                                <button
                                    ...
                                    className="text-sm text-text-secondary hover:text-foreground flex items-center gap-1 px-2 py-1 rounded hover:bg-glass transition-colors"
                                    title={tc("clearPrompt")}
                                >
                                    <Eraser size={12} /> {tc("clear")}
                                </button>
                            </div>
                        </div>
                                ...
                                {[0, 1, 2].map((idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={!isActive}
                                            className={`text-sm px-2 py-1 rounded-lg border transition-all flex items-center gap-1.5 ${isActive
                                        ...
                                        >
                                            {video?.thumbnail ? (
                                                ...
                                                <span className="w-4 h-4 rounded-full bg-primary/30 flex items-center justify-center text-sm">+</span>
                                            )}
                                            <span>{tc("insertCharacter", { name: slot?.name || tc("characterShort", { index: idx + 1 }) })}</span>
                                        </button>
                                    );
                                })}
                            </div>
                            ...
                            {polishedPrompt && (
                                ...
                                >
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between items-center">
                                            <span className="text-sm font-medium text-text-muted uppercase">{tc("cnPreview")}</span>
                                            <button
                                                onClick={() => {
                                        ...
                                        <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap bg-surface p-2 rounded">
                                            {polishedPrompt.cn}
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between items-center">
                                            <span className="text-sm font-medium text-text-muted uppercase">{tc("enGeneration")}</span>
                                            <div className="flex gap-1">
                                                ...
                                                <button
                                                    ...
                                                    className="text-sm text-foreground bg-primary hover:bg-primary/90 px-2 py-0.5 rounded font-medium"
                                                >
                                                    {tc("apply")}
                                                </button>
                                            </div>
                                        </div>
                                        <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap bg-surface p-2 rounded font-mono">
                                            {polishedPrompt.en}
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-2 pt-2 border-t border-primary/20">
                                        <div className="flex gap-2">
                                            ...
                                            <button
                                                ...
                                                className="text-sm text-foreground bg-primary hover:bg-primary/90 px-3 py-1.5 rounded font-medium flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                                            >
            ...
            < div className="p-6 border-t border-glass-border bg-surface z-10" >
                <div className="max-w-4xl mx-auto w-full">
                    <button
                        ...
                        className={`w-full py-4 rounded-xl font-medium text-xl flex items-center justify-center gap-2 transition-all transform active:scale-[0.99] ${submitSuccess
                            ? "bg-status-completed-bg text-white"
                    ...
                    <div className="flex justify-center mt-3">
                        <label className="flex items-center gap-2 text-sm text-text-muted cursor-pointer hover:text-text-secondary">
