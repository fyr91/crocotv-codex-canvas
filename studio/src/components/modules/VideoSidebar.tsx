            ...
            <div className="flex border-b border-border-subtle">
                ...
                >
                    ...
                    {tasks.filter(t => t.status === "pending" || t.status === "processing").length > 0 && (
                        <span className="bg-primary text-white text-sm px-1.5 rounded-full">
                        ...
                        >
                            ...
                            <section className="space-y-4">
                                ...
                                {(() => {
                                    ...
                                    if (durationConfig.type === 'fixed') {
                                        return (
                                            <div>
                                                <label className="block text-sm text-text-secondary mb-2">{tm("durationLabel")}</label>
                                                <div className="py-1.5 text-sm text-text-muted bg-glass rounded-lg text-center border border-transparent">
                                            ...
                                            <div className={`grid ${GRID_COLS_CLASS[durationConfig.options.length] ?? 'grid-cols-3'} gap-2`}>
                                                {durationConfig.options.map(dur => (
                                                    <button
                                                        ...
                                                        className={`py-1.5 text-sm rounded-lg border transition-all ${params.duration === dur
                                ...
                                {modelParams.shotType && (
                                    <div>
                                        <label className="block text-sm text-text-secondary mb-2">
                                            {tm("shotType")}
                                            {!params.promptExtend && (
                                                <span className="text-status-starred-fg ml-2">{tm("needPromptExtend")}</span>
                                            )}
                                        </label>
                                        <div className="grid grid-cols-2 gap-2">
                                            <button
                                                ...
                                                className={`py-2 text-sm rounded-lg border transition-all flex flex-col items-center gap-1 ${params.shotType === "single"
                                                    ? "bg-primary/20 border-primary text-primary"
                                                    : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                            ...
                                            <button
                                                ...
                                                disabled={!params.promptExtend}
                                                className={`py-2 text-sm rounded-lg border transition-all flex flex-col items-center gap-1 ${params.shotType === "multi"
                                                    ? "bg-primary/20 border-primary text-primary"
                                                    : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                    } ${!params.promptExtend ? 'opacity-50 cursor-not-allowed' : ''}`}
                                            >
                                ...
                                {modelParams.mode && (
                                    <div>
                                        <label className="block text-sm text-text-secondary mb-2">{tm("modeLabel")}</label>
                                        <div className="grid grid-cols-2 gap-2">
                                            {modelParams.mode.options.map(opt => (
                                                <button
                                                    key={opt}
                                                    onClick={() => updateParam("mode", opt)}
                                                    className={`py-1.5 text-sm rounded-lg border transition-all ${params.mode === opt
                                                        ? "bg-primary/20 border-primary text-primary"
                                                        : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                        }`}
                                                >
                            ...
                            <section className="space-y-4">
                                ...
                                {modelParams.resolution && (
                                    <div>
                                        <label className="block text-sm text-text-secondary mb-2">{tm("resolutionLabel")}</label>
                                        <div className={`grid ${GRID_COLS_CLASS[modelParams.resolution.options.length] ?? 'grid-cols-3'} gap-2`}>
                                            {modelParams.resolution.options.map(res => (
                                                <button
                                                    ...
                                                    className={`py-1.5 text-sm rounded-lg border transition-all ${params.resolution === res
                                                        ? "bg-primary/20 border-primary text-primary"
                                                        : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                        }`}
                                                >
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {[1, 2, 4].map(size => (
                                            <button
                                                ...
                                                className={`py-1.5 text-sm rounded-lg border transition-all ${params.batchSize === size
                                                    ? "bg-primary/20 border-primary text-primary"
                                                    : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                    }`}
                                            >
                                ...
                                {modelParams.cfgScale && (
                                    <div>
                                        <label className="block text-sm text-text-secondary mb-2">
                                            {tm("cfgScale")} <span className="text-primary font-medium">{params.cfgScale.toFixed(1)}</span>
                                        </label>
                                        <input
                                            type="range"
                                            ...
                                            className="w-full h-1.5 bg-hover-bg rounded-full appearance-none cursor-pointer accent-blue-500 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-primary [&::-webkit-slider-thumb]:shadow-lg"
                                        />
                                        <div className="flex justify-between text-sm text-text-muted mt-1">
                                ...
                                {modelParams.movementAmplitude && (
                                    <div>
                                        <label className="block text-sm text-text-secondary mb-2">{tm("movementAmplitude")}</label>
                                        <div className={`grid ${GRID_COLS_CLASS[modelParams.movementAmplitude.options.length] ?? 'grid-cols-4'} gap-2`}>
                                            {modelParams.movementAmplitude.options.map(opt => (
                                                <button
                                                    ...
                                                    className={`py-1.5 text-sm rounded-lg border transition-all capitalize ${params.movementAmplitude === opt
                                                        ? "bg-primary/20 border-primary text-primary"
                                                        : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                        }`}
                                                >
                                ...
                                {modelParams.promptExtend && (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            className={`w-10 h-5 rounded-full relative transition-colors ${params.promptExtend ? "bg-primary" : "bg-hover-bg"}`}
                                        >
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-3 gap-2 mb-2">
                                            <button
                                                ...
                                                className={`py-1.5 text-sm rounded-lg border flex items-center justify-center gap-1 transition-all ${audioMode === "mute"
                                                    ? "bg-primary/20 border-primary text-primary"
                                                    : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                    }`}
                                            >
                                            ...
                                            <button
                                                ...
                                                className={`py-1.5 text-sm rounded-lg border flex items-center justify-center gap-1 transition-all ${audioMode === "ai"
                                                    ? "bg-primary/20 border-primary text-primary"
                                                    : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                    }`}
                                            >
                                            ...
                                            <button
                                                ...
                                                className={`py-1.5 text-sm rounded-lg border flex items-center justify-center gap-1 transition-all ${audioMode === "custom"
                                                    ? "bg-primary/20 border-primary text-primary"
                                                    : "bg-glass border-transparent text-text-secondary hover:bg-hover-bg"
                                                    }`}
                                            >
                                ...
                                {modelParams.sound && (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            className={`w-10 h-5 rounded-full relative transition-colors ${params.sound ? "bg-primary" : "bg-hover-bg"}`}
                                        >
                                ...
                                {modelParams.viduAudio && (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            className={`w-10 h-5 rounded-full relative transition-colors ${params.viduAudio ? "bg-primary" : "bg-hover-bg"}`}
                                        >
                                ...
                                {modelParams.negativePrompt && (
                                    <div>
                                        <button
                                            ...
                                            className="text-sm text-text-muted hover:text-text-secondary flex items-center gap-1 mb-2"
                                        >
                            ...
                            <section className="space-y-4">
                                <h3 className="text-sm font-medium text-text-muted uppercase tracking-wider flex items-center gap-2">
                                    <div className="w-1 h-3 bg-status-processing-bg rounded-full" />
                                    {tm("advancedLabel")}
                                </h3>
                                ...
                                {modelParams.seed && (
                                    <div>
                                        <label className="block text-sm text-text-secondary mb-2">{tm("seedLabel")}</label>
                                        <div className="relative">
                                            <input
                                                type="number"
                                                value={params.seed ?? ""}
