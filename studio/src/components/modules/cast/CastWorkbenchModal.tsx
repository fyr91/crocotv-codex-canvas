...
export default function CastWorkbenchModal({ isOpen, kind, entityId, onClose }: CastWorkbenchModalProps) {
    ...
    const accentClasses = {
        character: {
            headerPill: "bg-primary/15 text-primary border-primary/30",
            batchActive: "border-primary/60 bg-primary/15 text-primary",
            variantSelected: "border-primary ring-2 ring-primary/40",
            selectBadge: "bg-primary",
        },
        scene: {
            headerPill: "bg-status-completed-bg text-status-completed-fg border-status-completed-border",
            batchActive: "border-status-completed-border bg-status-completed-bg text-status-completed-fg",
            variantSelected: "border-status-completed-border ring-2 ring-status-completed-border",
            selectBadge: "bg-status-completed-bg",
        },
        prop: {
            headerPill: "bg-status-processing-bg text-status-processing-fg border-status-processing-border",
            batchActive: "border-status-processing-border bg-status-processing-bg text-status-processing-fg",
            variantSelected: "border-status-processing-border ring-2 ring-status-processing-border",
            selectBadge: "bg-status-processing-bg",
        },
    } as const;
        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    className="w-[85vw] max-w-[96rem] h-[92vh] flex flex-col rounded-2xl border border-glass-border bg-elevated shadow-2xl overflow-hidden"
                ...
                >
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)_minmax(0,1.2fr)] divide-x divide-glass-border min-h-0">
                        ...
                        <div className="hidden md:flex flex-col gap-3 p-4 overflow-y-auto custom-scrollbar bg-surface/50">
                            {/* Entity metadata */}
                            <div>
                                <p className="font-mono text-sm uppercase tracking-[0.18em] text-text-muted mb-1">
                                    {t(`kind.${kind}`)}
                                </p>
                            ...
                            {(() => {
                                ...
                                return (
                                    <div className="pt-3 border-t border-glass-border">
                                        <p className="font-mono text-sm uppercase tracking-[0.18em] text-text-muted mb-1.5">
                                            {t("appearsIn")} ({appearsIn.length})
                                        </p>
                                        <div className="flex flex-wrap gap-1">
                                            {appearsIn.slice(0, 6).map((ep: any) => (
                                                <span key={ep.id} className="px-1.5 py-0.5 rounded bg-elevated border border-glass-border text-sm text-text-secondary truncate max-w-[110px]">
                                                    {ep.title}
                                                </span>
                                            ))}
                                        </div>
                            ...
                            <div className="pt-3 border-t border-glass-border">
                                <div className="flex items-center justify-between">
                                    <p className="flex items-center gap-1.5 font-mono text-sm uppercase tracking-[0.18em] text-text-muted">
                                ...
                                <p className="mt-1 text-sm text-foreground">{styleName || t("styleNotSet")}</p>
                                {!applyStyle && styleName && (
                                    <p className="text-sm text-status-processing-fg mt-0.5">{t("styleDisabledHint")}</p>
                                )}

                                {/* Positive prompt */}
                                {applyStyle && stylePositive && (
                                    <div className="mt-2.5 rounded-md bg-primary/5 border border-primary/10 px-2.5 py-2">
                                        <p className="font-mono text-sm uppercase tracking-[0.14em] text-primary/70 mb-1">{t("positiveLabel")}</p>
                                        <p className={`text-sm leading-relaxed text-text-secondary ${!positiveExpanded ? "line-clamp-3" : ""}`}>
                                            {stylePositive}
                                        </p>
                                        {stylePositive.length > 80 && (
                                            <button
                                                onClick={() => setPositiveExpanded(!positiveExpanded)}
                                                className="mt-1 text-sm text-primary/60 hover:text-primary/90 transition-colors"
                                            >
                                ...
                                {applyStyle && styleNegative && (
                                    <div className="mt-2 rounded-md bg-status-failed-bg border border-status-failed-border px-2.5 py-2">
                                        <p className="font-mono text-sm uppercase tracking-[0.14em] text-status-failed-fg mb-1">{t("negativeLabel")}</p>
                                        <p className={`text-sm leading-relaxed text-text-secondary ${!negativeExpanded ? "line-clamp-3" : ""}`}>
                                            {styleNegative}
                                        </p>
                                        {styleNegative.length > 80 && (
                                            <button
                                                onClick={() => setNegativeExpanded(!negativeExpanded)}
                                                className="mt-1 text-sm text-status-failed-fg hover:text-status-failed-fg transition-colors"
                                            >
                        ...
                        <div className="flex flex-col p-5 overflow-y-auto custom-scrollbar">
                                ...
                                <div className="mb-4">
                                    ...
                                    {pendingTemplate && (
                                        <div className="mt-2 flex items-center gap-2 px-2 py-1.5 rounded-md bg-status-processing-bg border border-status-processing-border">
                                            <span className="text-sm text-status-processing-fg">{t("tplSwitchConfirm")}</span>
                                            <button
                                                onClick={confirmTemplateSwitch}
                                                className="px-2 py-0.5 rounded text-sm font-medium bg-status-processing-bg text-status-processing-fg hover:bg-status-processing-bg transition-colors"
                                            >
                                                {t("tplSwitchYes")}
                                            </button>
                                            <button
                                                onClick={cancelTemplateSwitch}
                                                className="px-2 py-0.5 rounded text-sm text-text-muted hover:text-text-secondary transition-colors"
                                            >
                                                {t("tplSwitchNo")}
                                            </button>
                                        </div>
                            ...
                            <div className="flex items-center justify-between mb-2">
                                <label className="font-mono text-sm uppercase tracking-[0.18em] text-text-muted">
                                    {t("promptLabel")}
                                </label>
                                <button
                                    onClick={handleResetTemplate}
                                    disabled={generating}
                                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-sm text-text-muted hover:text-foreground transition-colors disabled:opacity-30"
                            ...
                            <textarea
                                ...
                                disabled={generating}
                                className="w-full min-h-[260px] max-h-[400px] rounded-md border border-glass-border bg-black/30 px-3.5 py-2.5 text-sm text-foreground placeholder:text-text-muted focus:outline-none focus:border-primary/40 disabled:opacity-60 resize-y leading-relaxed"
                            />
                            ...
                            {applyStyle && stylePositive && (
                                <div className="mt-3 rounded-md bg-black/20 border border-glass-border">
                                    ...
                                    >
                                        <p className="font-mono text-sm uppercase tracking-[0.14em] text-text-muted">{t("finalPromptPreview")}</p>
                                        <span className="text-sm text-text-muted">{finalPreviewExpanded ? t("collapse") : t("expand")}</span>
                                    </button>
                            ...
                            <div className="mt-5 pt-4 border-t border-glass-border space-y-4">
                                ...
                                {/* Batch — full row */}
                                <div>
                                    <label className="block font-mono text-sm uppercase tracking-[0.16em] text-text-muted mb-2">
                                        {t("batchLabel")}
                                    </label>
                            ...
                            <button
                                ...
                                className="mt-5 self-center inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-md bg-primary text-white border border-primary/65 shadow-none hover:bg-primary-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                            >
                        ...
                        <div className="flex flex-col p-5 overflow-y-auto custom-scrollbar bg-surface">
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="font-mono text-sm uppercase tracking-[0.18em] text-text-muted">
                                    {t("variantsTitle")}
                                ...
                                {variants.length > 0 && (
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <button
                                            ...
                                            className={`px-2.5 py-1 rounded text-sm transition-colors inline-flex items-center gap-1 ${
                                                galleryFilter === "favorited"
                                                    ? "bg-status-starred-bg text-status-starred-fg"
                                                    : "text-text-muted hover:text-text-secondary"
                                            }`}
                                        >
                            ...
                            {filteredVariants.length === 0 && variants.length === 0 ? (
                                <div className="flex-1 grid place-items-center text-center text-text-muted">
                                    <div className="max-w-xs">
                                        ...
                                        <p className="text-sm text-foreground">{t("emptyVariantsTitle")}</p>
                                        <p className="text-sm text-text-secondary mt-1">{t("emptyVariantsBody")}</p>
                                    </div>
                                </div>
                            ) : filteredVariants.length === 0 ? (
                                <div className="flex-1 grid place-items-center text-center text-text-muted">
                                    <p className="text-sm">{t("noFavoritedYet")}</p>
                                </div>
                            ) : (
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {!isSelected && (
                                                    ...
                                                    >
                                                        <p className="w-full text-center text-sm uppercase tracking-[0.16em] text-foreground font-mono">
                                                            {t("clickToSelect")}
                                                        </p>
                                                    </div>
                                                )}
