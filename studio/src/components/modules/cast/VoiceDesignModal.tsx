...
export default function VoiceDesignModal({
    ...
    const handleTranslate = async () => {
        ...
        try {
            const { voice_prompt } = await api.translateVoicePrompt(characterDescription, projectId);
    ...
    return (
        <div className="fixed inset-0 z-[110] grid place-items-center bg-overlay backdrop-blur-sm" onClick={handleClose}>
            <div
                className="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl border border-glass-border bg-elevated shadow-2xl"
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5 custom-scrollbar">
                    ...
                    {(characterName || characterDescription) && (
                        <div className="rounded-lg border border-glass-border bg-black/20 px-4 py-3">
                            ...
                            {characterDescription && (
                                <p className="mt-1 text-sm text-text-secondary leading-relaxed line-clamp-3">
                                    {characterDescription}
                                </p>
                            )}
                        </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1.5">
                            <label className="block font-mono text-sm uppercase tracking-[0.18em] text-text-muted">
                                {t("voicePromptLabel")}
                            </label>
                            {characterDescription?.trim() && (
                                <button
                                    ...
                                    disabled={inFlight}
                                    className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-primary/10 border border-primary/30 text-primary hover:bg-primary/15 transition-colors text-sm disabled:opacity-40"
                                >
                                    {phase === "translating" ? (
                        ...
                        <textarea
                            ...
                            rows={5}
                            maxLength={500}
                            className="w-full rounded-md border border-glass-border bg-black/30 px-3 py-2.5 text-sm text-foreground placeholder:text-text-muted focus:outline-none focus:border-primary/40 disabled:opacity-60 resize-none"
                        />
                        <p className="mt-1 text-right font-mono text-sm text-text-muted">
                    ...
                    {/* Preview text */}
                    <div>
                        <label className="block font-mono text-sm uppercase tracking-[0.18em] text-text-muted mb-1.5">
                            {t("previewTextLabel")}
                        </label>
                        <input
                            type="text"
                            ...
                            disabled={inFlight}
                            className="w-full rounded-md border border-glass-border bg-black/30 px-3 py-2.5 text-sm text-foreground placeholder:text-text-muted focus:outline-none focus:border-primary/40 disabled:opacity-60"
                        />
                    </div>
                    ...
                    <button
                        ...
                        className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-md bg-glass border border-primary/40 text-primary hover:bg-primary/10 transition-colors text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                        {phase === "previewing" ? (
                ...
                <div className="flex items-center justify-end gap-2 px-6 py-3 border-t border-glass-border">
                    <button
                        onClick={handleClose}
                        className="inline-flex items-center px-3 py-1.5 rounded-md bg-glass border border-glass-border text-text-secondary hover:text-foreground hover:bg-hover-bg transition-colors text-sm"
                    >
                        {t("cancel")}
                    </button>
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-md bg-primary text-white border border-primary/65 shadow-none hover:bg-primary-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                    >
            ...
            {confirmClose && (
                ...
                >
                    <div
                        className="w-full max-w-xs rounded-xl border border-glass-border bg-elevated p-5 shadow-2xl"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <p className="text-sm text-foreground font-medium mb-1">{t("confirmCloseTitle")}</p>
                        <p className="text-sm text-text-secondary mb-4">{t("confirmCloseBody")}</p>
                        <div className="flex items-center gap-2 justify-end">
                            <button
                                onClick={() => setConfirmClose(false)}
                                className="px-3 py-1.5 rounded-md bg-glass border border-glass-border text-text-secondary hover:text-foreground text-sm transition-colors"
                            >
                                {t("confirmCloseStay")}
                            </button>
                            <button
                                onClick={handleForceClose}
                                className="px-3 py-1.5 rounded-md bg-status-failed-bg border border-status-failed-border text-status-failed-fg hover:bg-status-failed-bg/80 text-sm font-medium transition-colors"
                            >
                                {t("confirmCloseLeave")}
                            </button>
                        </div>
