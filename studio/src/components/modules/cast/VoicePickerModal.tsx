...
export default function VoicePickerModal({
    ...
    return (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-overlay backdrop-blur-sm" onClick={onClose}>
            <div
                className="w-full max-w-4xl max-h-[85vh] flex flex-col rounded-2xl border border-glass-border bg-elevated shadow-2xl"
            ...
            >
                ...
                <div className="flex items-center gap-1 px-6 pt-3 border-b border-glass-border">
                    {[
                        ...
                        <button
                            ...
                            className={`relative px-3 pb-2 font-mono text-sm uppercase tracking-[0.16em] transition-colors ${
                                tab === tabDef.id
                                    ? "text-foreground"
                    ...
                    {!loading && !error && tab === "system" && (
                        <div className="space-y-6">
                            ...
                            {recommended.length > 0 && (
                                <section>
                                    <h3 className="mb-2 flex items-center gap-2 font-mono text-sm uppercase tracking-[0.18em] text-text-muted">
                ...
                <div className="flex items-center justify-between gap-3 px-6 py-3 border-t border-glass-border">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            onClick={onClose}
                            className="inline-flex items-center px-4 py-2 rounded-md bg-glass border border-glass-border text-text-secondary hover:text-foreground hover:bg-hover-bg transition-colors text-sm"
                        >
                            {t("cancel")}
                        </button>
                        <button
                            ...
                            className="inline-flex items-center px-4 py-2 rounded-md bg-primary text-white border border-primary/65 shadow-none hover:bg-primary-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                        >
                            {t("apply")}
                        </button>
                    </div>
        ...
        >
            <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground" title={voice.name}>
                        {voice.name}
                    </p>
                    <p className="mt-0.5 font-mono text-sm uppercase tracking-[0.14em] text-text-muted">
                        {voice.gender}
...
function CustomVoiceList({
    ...
    return (
        <div className="space-y-4">
            ...
            <button
                onClick={onCreate}
                className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-dashed border-primary/40 bg-primary/5 px-4 py-3 text-sm font-medium text-primary hover:bg-primary/10 hover:border-primary/60 transition-colors"
            >
                {createLabel}
            </button>
                    ...
                    {voices.map((cv) => {
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-sm font-medium text-foreground" title={cv.label}>
                                            {cv.label}
                                        </p>
                                        <p className="mt-0.5 font-mono text-sm uppercase tracking-[0.14em] text-text-muted">
