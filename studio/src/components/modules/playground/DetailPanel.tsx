        ...
        <div className="relative w-[40%] h-full overflow-y-auto border-l border-border-subtle">
          ...
          <div className="px-6 pt-6 pb-4 border-b border-border-subtle pr-14">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="font-mono text-sm bg-primary/15 text-primary rounded px-[6px] py-[2px] uppercase tracking-[0.1em]">
                {MODE_LABELS[generation.mode] || generation.mode}
              </span>
              <span className="font-mono text-sm text-text-muted uppercase tracking-[0.1em]">
                {generation.id.slice(0, 8)}
              </span>
            </div>
            <h2 className="font-display atelier-display text-xl font-medium tracking-tight text-foreground leading-tight">
              {generation.model_id}
            </h2>
            <p className="font-mono text-sm text-text-muted mt-1.5">
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            {paramEntries.length > 0 && (
              <div>
                ...
                <div className="rounded-xl bg-surface-inset border border-border-subtle p-4 grid grid-cols-2 gap-x-4 gap-y-3.5">
                  {paramEntries.map(([label, value], i) => (
                    <div key={label} className={i === 0 ? 'col-span-2' : ''}>
                      <div className="font-mono text-sm uppercase tracking-[0.08em] text-text-muted mb-1">
                        {label}
                      </div>
                      <div
