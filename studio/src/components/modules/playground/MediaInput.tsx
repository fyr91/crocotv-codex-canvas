...
const ACTION_BTN_CLASS =
  'flex-1 px-3 py-1.5 rounded-full text-sm border border-border-subtle ' +
...
function resolveMediaSrc(path: string): string {
  return resolvePlaygroundMediaUrl(path);
}
...
function SingleRefPreview({
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-surface-inset border border-border-subtle">
      <div className="group relative w-20 h-20 shrink-0 rounded-xl overflow-hidden bg-elevated border border-border-subtle">
        {video ? (
          <video
    ...
    return (
      <div className="space-y-2">
        ...
        >
          ...
          <span className="text-sm text-text-muted">{t(`media.hints.${config.hintKey}`)}</span>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {inputMedia.map((path, index) => (
              <div
                key={path + index}
                className="group relative w-[72px] h-[72px] rounded-xl overflow-hidden bg-elevated border border-border-subtle"
              >
                {isVideoPath(path) ? (
                  <video
                    src={resolveMediaSrc(path)}
                ...
                <div className="absolute bottom-0 left-0 right-0 px-1 py-0.5 bg-gradient-to-t from-black/75 to-transparent text-sm text-white truncate">
                  {getFileName(path)}
                </div>
              </div>
            ))}
            ...
            {canAddMore && (
              <button
                ...
                disabled={uploading}
                className="
                  w-[72px] h-[72px] rounded-xl bg-input-bg
