  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className={`flex items-center gap-[10px] w-full text-left cursor-pointer glass-input hover:border-foreground/30 bg-surface-inset rounded-xl ${
          open ? 'shadow-none' : ''
        }`}
      >
        ...
        <span className="font-mono text-sm text-text-muted uppercase tracking-wider shrink-0">
          {selected?.family ?? ''}
        </span>
        <ChevronDown
      ...
      {open && (
        <div className="absolute top-full mt-1 w-full bg-elevated atelier-card border border-glass-border rounded-lg shadow-xl z-20 max-h-60 overflow-y-auto">
          {availableModels.length === 0 && (
            <div className="px-3 py-2 text-sm text-text-muted">{t('model.noModels')}</div>
          )}
            ...
            <div key={group.family}>
              ...
              {groupedModels.length > 1 && (
                <div className="px-3 pt-2 pb-1">
                  <span className="font-mono text-sm text-text-muted uppercase tracking-[0.15em]">
                    {group.family}
                  </span>
                </div>
              )}
              {group.models.map((m) => (
                ...
                >
                  ...
                  {m.recommended && (
                    <span className="recommendation-badge shrink-0">
                      {t('model.recommended')}
                    </span>
                  )}
                  {m.id === modelId && (
