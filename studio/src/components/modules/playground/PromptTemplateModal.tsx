    ...
    <div className="fixed inset-0 z-50" onClick={dismiss}>
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-border-subtle flex items-center gap-2 shrink-0">
          ...
          <h2 className="font-display atelier-display text-xl font-medium tracking-tight text-foreground">
          ...
          <>
              ...
              <div className="flex gap-[2px] p-[3px] bg-surface-inset rounded-full atelier-pill-tabs">
                {[{ value: "all" as const, labelKey: "template.filterAll" as const }, ...CATEGORIES].map((c) => (
                  <button
                    ...
                    className={[
                      "flex-1 rounded-full px-2 py-1.5 text-sm font-medium text-center cursor-pointer transition-all",
                      filterCat === c.value
            ...
            <div className="flex-1 overflow-y-auto px-6 py-2 min-h-0 space-y-2">
              {filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center px-6">
                  ...
                  <p className="font-display italic text-sm text-text-secondary mb-1.5 leading-relaxed">
                    {filterCat === "all"
                      ? t("template.emptyAll")
                      : t("template.emptyFiltered", { category: t(categoryMeta(filterCat).labelKey) })}
                  </p>
                  <p className="text-sm text-text-muted">{t("template.emptyHint")}</p>
                </div>
              ) : (
                filtered.map((tpl) => {
                      ...
                      <div className="flex items-start gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            <span className={`text-sm font-mono uppercase px-1.5 py-[2px] rounded bg-elevated shrink-0 ${meta.color}`}>
                              {t(meta.labelKey)}
                            </span>
                            {tpl.default_mode && (
                              <span className="text-sm font-mono uppercase px-1.5 py-[2px] rounded bg-glass text-text-muted shrink-0">
                                {tpl.default_mode}
                              </span>
                            )}
                          </div>
                          <p className="font-display italic text-sm text-text-secondary line-clamp-2 leading-[1.6]">
                            {tpl.prompt}
                          </p>
                        </div>
                        ...
                        <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            ...
                            className="h-7 px-2.5 rounded-md text-sm font-medium text-primary bg-primary/10 hover:bg-primary/20 transition-colors flex items-center gap-1"
                          >
                            <Copy size={11} />
                            {t("template.apply")}
                          </button>
            ...
            <div className="border-t border-border-subtle shrink-0 px-6 py-3 flex items-center">
              <button
                type="button"
                onClick={openCreate}
                className="inline-flex items-center gap-1.5 text-sm font-medium text-text-muted hover:text-foreground transition-colors"
              >
                <Plus size={14} />
              ...
              {currentPrompt.trim() && (
                <button
                  ...
                  className="ml-auto inline-flex items-center gap-1.5 text-sm font-medium text-text-muted hover:text-foreground transition-colors"
                >
                  <Copy size={11} />
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3 min-h-0">
              ...
              <input
                ...
                className="w-full h-9 px-3 text-sm bg-surface-inset border border-glass-border rounded-xl text-foreground placeholder:text-text-muted outline-none focus:border-primary transition-colors shrink-0"
              />
              ...
              <div className="flex gap-[2px] p-[3px] bg-surface-inset rounded-full atelier-pill-tabs shrink-0">
                {CATEGORIES.map((c) => (
                  <button
                    ...
                    className={[
                      "flex-1 py-[6px] rounded-full text-sm font-medium text-center cursor-pointer transition-all",
              ...
              <textarea
                ...
                className="w-full flex-1 min-h-[180px] px-3 py-2.5 text-sm leading-relaxed bg-surface-inset border border-glass-border rounded-xl text-foreground placeholder:text-text-muted outline-none focus:border-primary transition-colors resize-none"
              />
            </div>
            ...
            <div className="border-t border-border-subtle shrink-0 px-6 py-4">
              <button
                ...
                className={[
                  "w-full h-9 rounded-full text-sm font-medium transition-all",
                  form.name.trim() && form.prompt.trim()
                    ? "bg-primary text-on-accent hover:bg-primary-hover shadow-none"
