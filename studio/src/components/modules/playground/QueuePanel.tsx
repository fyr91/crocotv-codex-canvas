  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="inline-flex items-center gap-1.5 rounded-full border border-glass-border bg-surface-inset px-3.5 py-2 text-sm font-medium text-text-muted transition-colors cursor-pointer hover:bg-hover-bg hover:text-foreground"
        title={t('queue.label')}
      >
        <ListOrdered className="w-4 h-4" />
        {t('queue.label')}
        {waiting > 0 && <span className="font-mono text-sm text-accent">· {waiting}</span>}
      </button>

      {open && (
        <div className="absolute right-0 top-full z-40 mt-2 w-[300px] rounded-2xl border border-glass-border bg-elevated p-4 shadow-2xl">
          {/* Concurrency control */}
          ...
          <p className="mb-3 text-sm text-text-muted">{t('queue.runningHint', { count: activeCount })}</p>

          {/* Queued requests */}
          {queue.length === 0 ? (
            <p className="py-3 text-center text-sm text-text-muted">{t('queue.empty')}</p>
          ) : (
            <div className="max-h-64 space-y-1.5 overflow-y-auto">
              {queue.map((q) => (
                ...
                >
                  <span
                    className={`font-mono text-sm uppercase px-1.5 py-[1px] rounded shrink-0 ${
