    ...
    <div className="rounded-2xl border border-status-failed-border bg-glass overflow-hidden">
      ...
      >
        ...
        <div className="relative text-center px-4 py-3 w-full">
          <p className="font-mono text-sm text-status-failed-fg uppercase mb-2">{t('card.failed')}</p>
          {error && (
            <p className={`text-sm text-text-muted leading-relaxed break-all ${expanded ? '' : 'line-clamp-2'}`}>
              {error}
            </p>
          )}
        </div>
        ...
        <div className="relative flex items-center gap-2 pb-2">
          {onRetry && (
            <button
              ...
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded text-sm font-medium text-primary bg-primary/10 hover:bg-primary/20 transition-colors"
            >
              ↻ {t('card.retry')}
            </button>
          )}
          {onDelete && (
            <button
              ...
              className="inline-flex items-center gap-1 px-2 py-1 rounded text-sm font-medium text-status-failed-fg/60 hover:text-status-failed-fg hover:bg-status-failed-bg transition-colors"
            >
              × {t('card.delete')}
            </button>
          )}
          {error && (
            <button
              onClick={handleCopy}
              className="inline-flex items-center gap-1 px-2 py-1 rounded text-sm font-medium text-text-muted hover:text-foreground hover:bg-hover-bg transition-colors"
            >
      ...
      <div className="px-3 py-[10px]">
        <p className="text-sm text-text-secondary line-clamp-2 mb-1.5">{prompt}</p>
        <div className="flex items-center gap-2">
          <span className="font-mono text-sm bg-glass text-text-muted rounded px-[6px] py-[2px]">
            {model_id || mode}
          </span>
    ...
    >
      ...
      <div className="relative overflow-hidden bg-elevated" style={{ aspectRatio: '16/9' }}>
        {mediaUrl ? (
          isVideo ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-elevated to-surface flex flex-col items-center justify-center gap-1.5">
              ...
              <span className="font-mono text-sm text-text-muted/50">{t('card.imageUnavailable') || 'Unavailable'}</span>
            </div>
          ) : (
        ...
        {(featured || isVideo) && (
          <div className="absolute top-2 left-2 z-[3] flex items-center gap-1.5">
            {featured && (
              <span
                className="inline-flex items-center gap-1 font-mono text-sm uppercase tracking-[0.08em] bg-status-starred-bg text-status-starred-fg border border-status-starred-border rounded px-[6px] py-[2px] backdrop-blur-sm"
            ...
            {isVideo && (
              <span className="font-mono text-sm bg-black/60 text-foreground/80 backdrop-blur-sm rounded px-[6px] py-[2px] uppercase">
      ...
      <div className="px-3 py-[10px]">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {/* Mode badge */}
          <span className="font-mono text-sm bg-primary/10 text-primary/70 rounded px-[6px] py-[2px] uppercase">
            {MODE_LABELS[mode] || mode}
          </span>
      ...
      <div className="rounded-2xl border border-glass-border bg-glass atelier-asset-card overflow-hidden">
        ...
        <div className="relative overflow-hidden bg-elevated" style={{ aspectRatio: '16/9' }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
            ...
            <span className="font-mono text-sm text-text-muted uppercase">
