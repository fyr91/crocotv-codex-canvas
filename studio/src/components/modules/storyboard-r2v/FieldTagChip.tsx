...
const FIELD_COLORS: Record<FieldType, { bg: string; border: string; text: string; hoverBorder: string }> = {
    duration: {
        ...
        text: "text-status-completed-fg",
        hoverBorder: "hover:border-status-completed-border",
    },
    shotSize: {
        bg: "bg-primary/10",
    ...
    return (
        <div className="relative inline-flex">
            <button
                ...
                type="button"
                onClick={() => setOpen(v => !v)}
                className={`inline-flex items-center gap-1 rounded-full border border-glass-border bg-surface-inset px-2.5 py-1 text-sm font-medium text-text-secondary cursor-pointer transition-all duration-150 ease-out group/chip hover:border-foreground/20 hover:text-foreground ${
                    isEmpty
                        ? "border-dashed border-foreground/20 bg-glass text-text-muted hover:border-foreground/30 hover:text-text-secondary"
                        : ""
                }`}
            >
                <span className="font-mono text-sm uppercase tracking-[0.06em] text-text-muted">
                    {label}
                </span>
...
function DurationEditor({ min, max, step, value, onChange }: {
            ...
            <div className="grid grid-cols-4 gap-1">
                {options.map(n => (
                    <button
                        ...
                        type="button"
                        onClick={() => onChange(n)}
                        className={`rounded-md px-2 py-1.5 text-sm font-mono font-medium transition-colors ${
                            n === value
                                ? "bg-hover-bg border border-border-strong text-foreground"
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            />
            <div className="flex items-center justify-between text-sm font-mono text-text-muted">
                <span>{min}s</span>
                <span className="text-status-completed-fg font-medium">{local}s</span>
                <span>{max}s</span>
            </div>
        </div>
    );
        ...
        <div className="flex flex-col gap-1.5">
            ...
            {allowCustom && (
                <input
                    ...
                    className="w-full rounded-md border border-glass-border bg-black/30 px-2.5 py-1.5 text-sm text-foreground placeholder:text-text-muted outline-none focus:border-primary/40 transition-colors"
                />
            )}
        </div>
    );
    ...
    return (
        <div className="relative inline-flex">
            <button
                ...
                type="button"
                onClick={() => setOpen(v => !v)}
                className="inline-flex items-center gap-0.5 rounded-md border border-dashed border-foreground/15 px-1.5 py-1 text-sm text-text-muted hover:border-foreground/30 hover:text-text-secondary transition-colors cursor-pointer"
            >
                    ...
                    >
                        {fields.map(f => (
                            <button
                                ...
                                className="w-full text-left rounded-md px-2.5 py-1.5 text-sm text-text-secondary hover:bg-hover-bg hover:text-foreground transition-colors"
                            >
                                {f.label}
                            </button>
                        ))}
