    ...
    return (
        <div className="border-b border-glass-border bg-surface/60 backdrop-blur-sm">
            ...
            >
                ...
                <span className="font-mono text-sm font-medium uppercase tracking-[0.18em] text-text-secondary">
                    {t("title")}
                </span>
                {prevTitle && (
                    <span className="font-mono text-sm tracking-tight text-text-muted truncate max-w-[200px]">
                        · {prevTitle}
                    </span>
                )}
                {!open && frames.length > 0 && (
                    <span className="ml-auto font-mono text-sm uppercase tracking-[0.14em] text-text-muted">
            ...
            {open && (
                <div className="px-4 sm:px-6 pb-3 pt-1">
                    ...
                    <p className="mt-2 text-sm text-text-muted italic px-1">
                        {t("hint")}
                    </p>
                </div>
            )}
...
function FrameThumb({ frame, indexFromEnd }: { frame: FrameLite; indexFromEnd: number }) {
    ...
    return (
        <div className="shrink-0 w-[140px]">
            ...
            <p className="mt-1 font-mono text-sm uppercase tracking-[0.14em] text-text-muted">
                {t("frameLabel", { offset: indexFromEnd })}
            </p>
            {frame.action_description && (
                <p className="mt-0.5 text-sm text-text-secondary line-clamp-2 leading-snug">
                    {frame.action_description}
                </p>
            )}
        </div>
