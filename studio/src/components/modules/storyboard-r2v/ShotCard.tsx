...
export default function ShotCard({
    ...
    const renderPreview = () => {
        if (shot.tabMode === "t2i_i2v") {
            ...
            if (shot.videoStatus === "failed") {
                return (
                    <div className="w-full aspect-video flex flex-col items-center justify-center gap-2">
                        <span className="text-sm text-status-failed-fg font-medium">{t("generationFailed")}</span>
                        <button
            ...
            if (shot.t2iImageUrl) {
                ...
                return (
                    <div className="w-full aspect-video relative">
                        ...
                        />
                        <div className="absolute bottom-2 left-2 text-sm px-1.5 py-0.5 rounded-full bg-status-completed-bg/90 text-white font-medium backdrop-blur-sm pointer-events-none">
                            {t("generateVideoNext")}
                        </div>
                    </div>
                );
            ...
            return (
                <div className="w-full aspect-video flex flex-col items-center justify-center gap-2.5 text-text-muted">
                    ...
                    <span className="font-mono text-sm uppercase tracking-[0.08em]">{t("generateImageOrUpload")}</span>
                </div>
            );
        }
        ...
        return (
            <div className="w-full aspect-video flex flex-col items-center justify-center gap-2.5 text-text-muted">
                ...
                <span className="font-mono text-sm uppercase tracking-[0.08em]">{t("noVideoYet")}</span>
            </div>
        );
    };
    ...
    function ShotStatusBadge({ shot, t }: { shot: ShotNode; t: (key: string, values?: Record<string, number | string>) => string }) {
        ...
        if (isStarred) {
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-status-starred-border bg-status-starred-bg px-2.5 py-1 font-mono text-sm font-medium uppercase tracking-[0.1em] text-status-starred-fg">
        ...
        if (isProcessing) {
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-status-processing-border bg-status-processing-bg px-2.5 py-1 font-mono text-sm font-medium uppercase tracking-[0.1em] text-status-processing-fg">
        ...
        if (isPending) {
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-status-pending-border bg-status-pending-bg px-2.5 py-1 font-mono text-sm font-medium uppercase tracking-[0.1em] text-status-pending-fg">
        ...
        if (isFailed) {
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-status-failed-border bg-status-failed-bg px-2.5 py-1 font-mono text-sm font-medium uppercase tracking-[0.1em] text-status-failed-fg">
        ...
        return (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-glass-border bg-black/20 px-2.5 py-1 font-mono text-sm font-medium uppercase tracking-[0.1em] text-text-secondary">
    ...
    return (
        ...
        >
            ...
            <div className="relative overflow-hidden rounded-2xl border border-glass-border bg-surface shadow-lg transition-all duration-base ease-out-quart group-hover:-translate-y-1 group-hover:shadow-2xl z-10">
                ...
                <div className="flex items-center justify-between gap-4 px-5 pt-4 pb-3">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="font-display text-3xl font-medium leading-none text-text-secondary tracking-tight">
                            {String(index + 1).padStart(2, "0")}
                        </div>
                        <div className="font-mono text-sm uppercase tracking-[0.08em] text-text-muted leading-tight">
                            <span>{t("shot")}</span>
                            {shot.shotSize ? (
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <div className="relative inline-flex items-center p-[3px] bg-surface-inset rounded-full">
                            <motion.div
                                className="absolute top-[3px] bottom-[3px] rounded-full bg-elevated shadow-none"
                                initial={false}
                                animate={{
                ...
                <div className="flex px-5">
                    {/* Left: Preview */}
                    <div className="group/preview relative w-72 shrink-0 bg-surface-inset flex flex-col items-center justify-center overflow-hidden rounded-xl border border-glass-border shadow-none">
                        {renderPreview()}
                        ...
                        {(shot.isVideoPinned || shot.finalTakeId) && shot.videoUrl ? (
                            <div className="absolute top-2.5 left-2.5 z-10 flex items-center gap-1 rounded-full border border-status-starred-border bg-status-starred-bg/90 px-2 py-[3px] backdrop-blur-sm font-mono text-sm font-medium uppercase tracking-[0.08em] text-status-starred-fg">
                        ...
                        {/* Duration chip */}
                        {shot.duration ? (
                            <div className="absolute bottom-2.5 right-2.5 z-10 rounded-full bg-overlay/70 px-2 py-0.5 backdrop-blur-sm font-mono text-sm text-foreground">
                                {shot.duration}s
                            </div>
                        ) : null}
                        ...
                        {shot.isVideoPinned && shot.videoUrl && onUnpinVideo ? (
                            <div className="group/pin absolute top-2.5 right-2.5 z-20 flex items-center gap-1">
                                <span
                                    className="inline-flex items-center gap-1 rounded-full border border-primary/55 bg-primary/20 backdrop-blur-sm px-2 py-[2px] font-mono text-sm uppercase tracking-[0.14em] text-primary shadow-none"
                                ...
                                <button
                                    ...
                                    className="opacity-0 transition-opacity duration-fast ease-out-quart group-hover/pin:opacity-100 focus-visible:opacity-100 inline-flex items-center gap-1 rounded-full border border-foreground/15 bg-black/55 backdrop-blur-sm px-1.5 py-[2px] font-mono text-sm uppercase tracking-[0.14em] text-foreground/80 hover:text-foreground hover:border-foreground/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                                >
                    ...
                    <div className="flex-1 py-0.5 pl-5 pr-0 flex flex-col gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center -space-x-2">
                                    ...
                                    {castAvatars.length > 3 ? (
                                        <span className="grid h-[26px] w-[26px] place-items-center rounded-full border-2 border-surface bg-elevated font-mono text-sm font-medium text-text-secondary">
                                            +{castAvatars.length - 3}
                                        </span>
                                    ) : null}
                                </div>
                        ...
                        <div className="relative">
                            <textarea
                                ...
                                className="w-full resize-none bg-transparent border-l-2 border-glass-border pl-3.5 pr-8 py-1 text-sm leading-[1.7] text-foreground placeholder:text-text-muted focus:outline-none focus:border-l-primary/40 focus:bg-glass/30 transition-all duration-200 min-h-[80px] max-h-[260px] overflow-y-auto"
                                rows={5}
                            />
                        ...
                        {shot.dialogueStructured?.line && (
                            <div className="pl-3.5 border-l-2 border-accent/40">
                                <span className="block font-mono text-sm uppercase tracking-[0.08em] text-text-muted">
                                    {shot.dialogueStructured.speaker}
                                </span>
                                <span className="block font-display text-sm italic leading-snug text-text-secondary">
                                    “{shot.dialogueStructured.line}”
                                </span>
                            </div>
                        )}
                            ...
                            <div className="mt-1">
                                ...
                                <AnimatePresence>
                                    {promptPreviewOpen && (
                                        ...
                                        >
                                            <div className="mt-1.5 rounded-md border border-glass-border bg-black/20 px-3 py-2 text-sm leading-relaxed font-mono space-y-2">
                ...
                <div className="flex items-center justify-between gap-3 px-5 py-3.5 border-t border-border-subtle">
                            <div className="flex items-center gap-1 shrink-0">
                                <motion.button
                                    ...
                                    className="ico-btn flex h-8 w-8 items-center justify-center rounded-xl text-text-muted transition-colors duration-fast ease-out-quart hover:bg-hover-bg hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                                ...
                                <motion.button
                                    ...
                                    onClick={onMoveUp}
                                    disabled={index === 0}
                                    className="ico-btn flex h-8 w-8 items-center justify-center rounded-xl text-text-muted transition-colors duration-fast ease-out-quart hover:bg-hover-bg hover:text-foreground disabled:opacity-25 disabled:hover:bg-transparent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                                    title={t("moveUp")}
                                >
                                ...
                                {onRefineFrame && (
                                    <motion.button
                                        ...
                                        className="ico-btn flex h-8 w-8 items-center justify-center rounded-xl text-text-muted transition-colors duration-fast ease-out-quart hover:bg-hover-bg hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                                        title={t("refineFrame")}
                                    >
                                ...
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.94 }}
                                    onClick={onDelete}
                                    className="ico-btn flex h-8 w-8 items-center justify-center rounded-xl text-text-muted transition-colors duration-fast ease-out-quart hover:bg-hover-bg hover:text-status-failed-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                                    title={t("deleteShot")}
                                >
                            ...
                            <div className="flex items-center gap-3">
                                {genSummary && (
                                    <span className="inline-flex items-center gap-1.5 shrink-0 font-mono text-sm text-text-secondary">
                                        <span className="h-1.5 w-1.5 rounded-full bg-primary shadow-none" />
                                        {genSummary}
                                    </span>
                                )}
                                <span className="font-mono text-sm uppercase tracking-[0.1em] text-text-muted hidden sm:inline">
                                    {t("countLabel")}
                                </span>
                                    ...
                                    {[1, 2, 3].map((n) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`grid h-7 min-w-[28px] place-items-center rounded-full font-mono text-sm font-medium transition-colors duration-fast ease-out-quart focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55 ${
                                                    active
                                ...
                                <motion.button
                                    ...
                                    className="inline-flex items-center justify-center gap-1.5 rounded-full px-[13px] py-[7px] font-sans text-sm font-medium tracking-tight transition-all duration-fast ease-out-quart focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55 disabled:cursor-not-allowed disabled:opacity-40 bg-primary text-on-accent shadow-none hover:bg-primary-hover hover:-translate-y-px disabled:hover:translate-y-0"
                                >
                                    {inFlightCount > 0 ? (
                                        <>
                        ...
                        <button
                            ...
                            className="group/disc flex w-full items-center gap-2.5 border-t border-border-subtle px-5 py-2.5 font-mono text-sm uppercase tracking-[0.14em] text-text-muted transition-colors duration-fast ease-out-quart hover:bg-hover-bg hover:text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                        >
                            {expanded ? (
                            ...
                            <span className="text-foreground font-medium">{expanded ? t("collapseShotShort") : t("expandShotShort")}</span>
