        ...
        >
            <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:gap-5">
                ...
                <div className="min-w-0 flex-1 space-y-1.5">
                    ...
                    <div className="font-display text-display-sm font-medium text-foreground">
                        {t("t2iHeroTitle")}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-2">
                        <button
                            ...
                            className="btn-tip inline-flex min-h-[36px] items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 font-display text-display-sm font-medium text-white shadow-none transition-all duration-fast ease-out-quart hover:bg-primary/92 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            {generating ? (
                ...
                <div className="ml-auto flex items-center gap-0.5">
                    ...
                    >
                        ...
                        {menuOpen ? (
                            <div
                                role="menu"
                                className="absolute right-0 top-full z-10 mt-1 w-44 overflow-hidden rounded-md border border-glass-border bg-surface/96 shadow-lg backdrop-blur-md motion-safe:animate-[shotPanelIn_180ms_cubic-bezier(0.22,1,0.36,1)_both]"
                            >
                                <button
                                    type="button"
                                    role="menuitem"
    ...
    return (
        <span className="inline-flex items-center gap-1.5">
            <span
                aria-hidden="true"
                className={clsx(
                    "grid h-5 w-5 place-items-center rounded-full font-mono text-sm font-medium transition-colors duration-fast ease-out-quart",
                    done
...
function SinglePreview({ url, isStoryboard, onRemove, storyboardBadgeText }: SinglePreviewProps) {
    ...
    return (
        ...
        >
            ...
            {isStoryboard ? (
                <span
                    className="pointer-events-none absolute left-1 top-1 inline-flex items-center gap-1 rounded bg-black/65 px-1.5 py-0.5 font-mono text-sm font-medium uppercase text-text-secondary"
                    title={storyboardBadgeText}
                >
                    <Pin size={9} aria-hidden="true" />
                    {t("t2iStoryboardBadge")}
                </span>
            ) : null}
            {onRemove && hovered ? (
                <button
            ...
            <div className="relative h-[100px] w-[140px] shrink-0 overflow-hidden rounded-md border border-glass-border bg-black/40">
                ...
                {generating && !activeUrl ? (
                    <div className="absolute inset-0 grid place-items-center bg-black/65 backdrop-blur-[1px]">
                        <PendingTaskAffordance
                            statusLabel={inFlightStatus === "pending" ? t("statusPending") : t("statusProcessing")}
                            taskId={inFlightTaskId}
                            compact
                        />
                    </div>
