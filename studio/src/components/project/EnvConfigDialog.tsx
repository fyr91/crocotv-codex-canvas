...
export default function EnvConfigDialog({ isOpen, onClose, isRequired = false }: EnvConfigDialogProps) {
  ...
  const modeButtonClass = (active: boolean) =>
    `px-3 py-1.5 text-sm rounded-md border transition-colors font-medium ${active ? "bg-hover-bg text-foreground border-border-strong shadow-none" : "border-glass-border bg-surface text-text-secondary hover:text-foreground"}`;
        ...
        >
          <div className="flex items-center justify-between p-6 border-b border-glass-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-medium text-foreground">{t("envConfig")}</h2>
                <p className="text-sm text-text-muted">{t("envConfigSub")}</p>
              </div>
            </div>
            <button
              onClick={requestClose}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {isRequired && (
              <div className="bg-status-starred-bg border border-status-starred-border rounded-lg p-3 text-sm text-status-starred-fg">
                {t("requiredHint")}
              </div>
            )}
            {isRequired && !canClose && (
              <div className="bg-glass border border-glass-border rounded-lg p-3 text-sm text-text-secondary">
                {t("cannotClose")}
              </div>
            )}
            ...
            {loading ? (
              <div className="flex items-center justify-center py-12">
                <Loader2 size={24} className="animate-spin text-status-processing-fg" />
              ...
              <>
                <div>
                  <label className="flex items-center justify-between text-sm font-medium text-foreground mb-2">
                    <span>DashScope API Key <span className="text-status-failed-fg">*</span></span>
                    <span className="text-text-muted font-normal text-sm">e.g. sk-xxx</span>
                  </label>
                  <input
                    type="password"
                ...
                <div className="pt-4 border-t border-glass-border">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="text-sm font-medium text-foreground">{t("ossMirror")}</h3>
                      <p className="text-sm text-text-muted mt-1">{t("ossMirrorDesc")}</p>
                    </div>
                    <a
                  ...
                  <div className="bg-glass border border-glass-border rounded-lg p-4 space-y-4">
                    ...
                    {config.KLING_PROVIDER_MODE === "vendor" && (
                      <>
                        <div>
                          <label className="block text-sm font-medium text-foreground mb-2">
                            Kling Access Key <span className="text-status-failed-fg">*</span>
                          </label>
                          <input
                            type="password"
                        ...
                        <div>
                          <label className="block text-sm font-medium text-foreground mb-2">
                            Kling Secret Key <span className="text-status-failed-fg">*</span>
                          </label>
                          <input
                            type="password"
                ...
                <div className="pt-4 border-t border-glass-border">
                  ...
                  <div className="bg-input-bg border border-glass-border rounded-lg p-4 space-y-4">
                    ...
                    {config.VIDU_PROVIDER_MODE === "vendor" && (
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">
                          Vidu API Key <span className="text-status-failed-fg">*</span>
                        </label>
                        <input
                          type="password"
          ...
          <div className="flex justify-end gap-3 p-6 border-t border-glass-border">
            ...
            <button
              ...
              className="flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-normal text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
            >
              {saving ? (
                <>
