    ...
    return (
        ...
        >
            ...
            <header className="flex shrink-0 items-center gap-3 border-b border-glass-border bg-surface px-8 py-5">
                <div className="grid h-9 w-9 place-items-center rounded-full border border-primary/32 bg-primary/08 text-primary">
            ...
            <div className="flex-1 overflow-y-auto bg-surface px-8 py-6 space-y-8 custom-scrollbar">
                {isLoading ? (
                    ...
                    <>
                        {/* Selected preview */}
                        <section>
                            <h3 className="mb-3 inline-flex items-center gap-2 font-mono text-sm font-medium uppercase tracking-[0.18em] text-text-secondary">
    ...
    return (
        ...
        >
            <motion.div
                ...
                className="w-[70vw] max-w-[1100px] min-w-[640px] max-h-[90vh] rounded-2xl border border-glass-border bg-elevated shadow-2xl overflow-hidden flex flex-col"
            ...
            >
                    ...
                    <div className="p-6 space-y-5 overflow-y-auto">
                        ...
                        {preset.best_for && preset.best_for.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {preset.best_for.map((tag, i) => (
                                    <span key={i} className="text-sm px-2.5 py-1 rounded-md bg-status-completed-bg text-status-completed-fg border border-status-completed-border">{tag}</span>
                                ))}
                                {preset.avoid_for?.map((tag, i) => (
                                    <span key={`avoid-${i}`} className="text-sm px-2.5 py-1 rounded-md bg-status-failed-bg text-status-failed-fg border border-status-failed-border line-through">{tag}</span>
                                ))}
                            </div>
                        )}
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <p className="text-sm uppercase tracking-wider text-text-muted font-medium">{t("promptLabel")}</p>
                                {!editing && (
                                    <button onClick={onStartEditing} className="flex items-center gap-1.5 text-sm text-text-muted hover:text-foreground transition-colors">
                                        <Pencil size={12} /><span>{t("customize")}</span>
                                    </button>
                                )}
                                {editing && isCustomized && (
                                    <span className="text-sm text-status-processing-fg font-medium">{t("modified")}</span>
                                )}
                            </div>
                ...
                {sameCategoryPresets.length > 0 && (
                    <div className="border-t border-glass-border px-6 py-3 shrink-0">
                        <p className="text-sm uppercase tracking-wider text-text-muted mb-2">{t("similarStyles")}</p>
