      ...
      <div className="flex-1 overflow-y-auto px-8 pb-8">
        {assets.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-text-secondary">
            ...
            <p className="text-sm text-text-muted mt-1">{t("assetsSharedHint")}</p>
          </div>
        ) : (
          <motion.div
    ...
    >
      ...
      <div className="px-8 pt-6 pb-4 flex items-start justify-between border-b border-glass-border">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <span className="text-sm bg-primary/20 text-primary px-2.5 py-1 rounded-lg font-mono font-medium">
          ...
          <p className="text-sm text-text-secondary">
            {episode.workflow_mode === "r2v" ? tp("workflowR2V") : tp("workflowI2V")} · {t("frameCount", { count: frames.length })}
          </p>
        </div>
        <motion.button
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
        ...
        <div className="space-y-2">
          <h3 className="text-sm font-medium text-foreground">{t("scriptSummary")}</h3>
          {originalText ? (
            <p className="text-sm text-text-secondary leading-relaxed line-clamp-4 bg-surface rounded-lg p-3 border border-glass-border">
              {originalText.slice(0, 300)}{originalText.length > 300 ? "..." : ""}
            </p>
          ) : (
            <p className="text-sm text-text-muted italic">{t("noScript")}</p>
          )}
        </div>

        {/* Storyboard Overview */}
        <div className="space-y-2">
          <h3 className="text-sm font-medium text-foreground">{t("storyboardOverview")}</h3>
          {frames.length === 0 ? (
            <div className="flex items-center gap-3 bg-surface rounded-lg p-4 border border-glass-border">
              ...
              <div>
                <p className="text-sm font-medium text-text-secondary">{t("noFrames")}</p>
                <p className="text-sm text-text-muted">{t("startCreating")}</p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-4 lg:grid-cols-6 gap-2">
              {frames.slice(0, 12).map((frame, i) => (
                ...
                >
                  {frame.rendered_image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-sm text-text-muted font-mono">
                      #{i + 1}
                    </div>
                  )}
                </div>
              ...
              {frames.length > 12 && (
                <div className="aspect-video bg-surface rounded-lg border border-glass-border flex items-center justify-center text-sm text-text-muted">
                  +{frames.length - 12}
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-surface rounded-lg p-3 border border-glass-border text-center">
            <p className="text-xl font-medium text-foreground">{characters.length}</p>
            <p className="text-sm text-text-muted">{t("characters")}</p>
          </div>
          <div className="bg-surface rounded-lg p-3 border border-glass-border text-center">
            <p className="text-xl font-medium text-foreground">{scenes.length}</p>
            <p className="text-sm text-text-muted">{t("scenes")}</p>
          </div>
          <div className="bg-surface rounded-lg p-3 border border-glass-border text-center">
            <p className="text-xl font-medium text-foreground">{frames.length}</p>
            <p className="text-sm text-text-muted">{t("storyboardFrames")}</p>
          </div>
        </div>
      </div>
    </motion.div>
