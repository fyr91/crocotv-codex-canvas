  ...
  return (
    <div className="mb-6 last:mb-0">
      <div className="mb-2.5">
        ...
        {hint && <div className="text-sm text-text-muted mt-1 leading-relaxed">{hint}</div>}
      </div>
...
export function FieldLabel({ children }: { children: ReactNode }) {
  return (
    <label className="block font-mono text-sm uppercase tracking-[0.1em] text-text-muted mb-2">
      {children}
    </label>
  );
}
...
export const settingsInputClass =
  "w-full bg-input-bg border border-glass-border rounded-md px-3.5 py-2.5 text-sm text-foreground placeholder-text-muted focus:outline-none focus:border-primary/50 transition-colors";
    ...
    <div className="atelier-field">
      <div className="relative">
        <input
          ...
          className={settingsInputClass + " pr-[78px] font-mono text-sm tracking-wide"}
        />
        <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex gap-0.5">
          ...
          <button
            ...
            className="w-7 h-7 rounded-md grid place-items-center text-text-muted hover:bg-hover-bg hover:text-foreground transition-colors"
          >
            {copied ? <Check size={14} className="text-status-completed-fg" /> : <Copy size={14} />}
          </button>
        </div>
      </div>
      {status && (
        <span
          className={`inline-flex items-center gap-1.5 font-mono text-sm tracking-wide mt-1.5 ${
            status.kind === "ok" ? "text-status-completed-fg" : "text-status-processing-fg"
          }`}
        >
  ...
  return (
    <div className="flex items-center gap-3.5">
      ...
      <div className="flex-1">
        ...
        {sub && <div className="text-sm text-text-muted mt-1 leading-relaxed">{sub}</div>}
      </div>
    </div>
  );
}
