...
export function PendingTaskAffordance({
    ...
    const elapsedClass = compact ? "text-sm" : "text-sm";
    const statusClass = compact ? "text-sm" : "text-sm";
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-1.5">
            ...
            />
            <span className={`${statusClass} font-medium text-status-processing-fg`}>
                {statusLabel}
            </span>
            ...
            {cancelError ? (
                <span className="font-mono text-sm text-status-failed-fg">
                    {cancelError}
                </span>
            ) : null}
            {diagnoseOpen ? (
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
    return (
        <div className="flex items-center justify-between gap-3">
            <span className="font-mono text-sm font-medium uppercase tracking-[0.24em] text-text-muted/85">
                {label}
            </span>
