...
import { resolveImageSizeSelection } from "@/lib/image-generation-size";
import { useThemeStore } from "@/stores/use-theme-store";
import { imageSizePresetsForModel, modelSupportsImagePromptOptimize, type AiConfig } from "@/stores/use-config-store";
...
export function CanvasImageSettingsPopover({ config, onConfigChange, onOpenChange, buttonClassName, placement = "topLeft" }: CanvasImageSettingsPopoverProps) {
    ...
    const selection = resolveImageSizeSelection(imageSizePresetsForModel(config.model), config.size);
    const optimizePrompt = modelSupportsImagePromptOptimize(config.model) ? ` · 提示词优化${config.imagePromptOptimize === "true" ? "开" : "关"}` : "";
            ...
            <span ref={buttonRef} className="inline-flex min-w-0">
                <Button size="small" type="text" className={buttonClassName || "!h-8 !max-w-[180px] !justify-start !rounded-full !px-2.5"} style={{ background: theme.node.fill, color: theme.node.text }} icon={<Settings2 className="size-3.5" />} onClick={() => updateOpen(!open)}>
                    <span className="truncate">
                        {selection.resolution} · {selection.ratio === "auto" ? "Auto" : selection.ratio} · {count} 张{optimizePrompt}
                    </span>
                </Button>
            </span>
            {panel}
