        ...
        <Card
            ...
            cover={
                <button type="button" className="block w-full text-left" onClick={onOpen}>
                    <PromptCover src={item.coverUrl} alt={item.title} />
                </button>
            }
        >
            <button type="button" className="block w-full text-left" onClick={onOpen}>
                <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="shrink-0 text-xs text-stone-400 dark:text-stone-500">{item.creatorName ? `来自 ${item.creatorName}` : formatPromptDate(item.updatedAt)}</span>
                    </div>
