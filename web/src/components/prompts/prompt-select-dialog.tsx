                    ...
                    <div className="grid gap-2 sm:grid-cols-[56px_minmax(0,1fr)] sm:items-start">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {promptCategories.map((category) => (
                                <Tag.CheckableTag key={category} checked={selectedCategory === category} className={cn("filter-chip", selectedCategory === category && "is-active")} onChange={() => setSelectedCategory(category)}>
                                    {category}
                                </Tag.CheckableTag>
                            ))}
                        </div>
                    ...
                    <div className="grid gap-2 sm:grid-cols-[56px_minmax(0,1fr)] sm:items-start">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {promptTags.map((tag) => {
                                ...
                                return (
                                    <Tag.CheckableTag key={tag} checked={active} className={cn("filter-chip", active && "is-active")} onChange={() => toggleTag(tag)}>
                                        {tag}
                                    </Tag.CheckableTag>
                                );
                            })}
