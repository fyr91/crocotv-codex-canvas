...
import { listSharedPrompts, type SharedPromptNodeType } from "@/services/api/shared-prompts";
...
export const CROCOTV_PROMPT_CATEGORY = "crocotv";
...
const promptCacheStore = localforage.createInstance({ name: "crocotv", storeName: "prompt_cache" });
...
export async function fetchPrompts({ keyword = "", tag = [], category = ALL_PROMPTS_OPTION, page = 1, pageSize = 20 }: { keyword?: string; tag?: string[]; category?: string; page?: number; pageSize?: number } = {}) {
    const [thirdPartyItems, sharedItems] = await Promise.all([getPrompts(), listSharedPrompts()]);
    const items: Prompt[] = [
        ...sharedItems.map((item) => ({ id: item.id, title: item.title, coverUrl: "", prompt: item.prompt, tags: [], category: CROCOTV_PROMPT_CATEGORY, githubUrl: "", preview: "", createdAt: item.createdAt, updatedAt: item.updatedAt, creatorName: item.creatorName, sourceNodeType: item.sourceNodeType })),
        ...thirdPartyItems,
    ];
    ...
    return {
        ...
        categories: [CROCOTV_PROMPT_CATEGORY, ...categories.map((item) => item.category)],
...
export function extractPromptImages(baseUrl: string, markup: string) {
    const matches = [
        ...Array.from(markup.matchAll(/!\[[^\]]*]\(([^)]+)\)/g), (match) => ({ index: match.index ?? 0, image: match[1] })),
        ...Array.from(markup.matchAll(/<img\b[^>]*\bsrc\s*=\s*(["'])(.*?)\1[^>]*>/gi), (match) => ({ index: match.index ?? 0, image: match[2] })),
    ].sort((a, b) => a.index - b.index);
    return Array.from(new Set(matches.map((item) => absoluteImage(baseUrl, item.image.trim())).filter(Boolean)));
}
