...
type RequestOptions = { signal?: AbortSignal; clientRequestId?: string; ltxFrames?: unknown; onJobCreated?: (jobId: string) => void; onStatusChange?: (status: "queued" | "running" | "succeeded" | "failed" | "canceled") => void; onProgress?: (progress: number, stage?: string) => void; onResult?: (result: VideoGenerationResult) => void; onArchived?: (result: VideoGenerationResult) => void; onReviewReady?: (review: never) => void };
...
export async function requestVideoGeneration(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videoReferences: ReferenceVideo[] = [], audioReferences: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationResult[]> {
    ...
    results.forEach((result) => options?.onResult?.(result)); options?.onProgress?.(100, "完成"); options?.onStatusChange?.("succeeded"); return results;
}
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videos: ReferenceVideo[] = [], audios: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationTask> { await requestVideoGeneration(config, prompt, references, videos, audios, options); return { id: crypto.randomUUID(), provider: providerIdForModel(config.videoModel || config.model) || "minimax_h3", model: modelOptionName(config.videoModel || config.model), expectedCount: 1, reviewMode: "none" }; }
